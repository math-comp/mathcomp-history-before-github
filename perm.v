...
Variable (G : finGroupType) (H : set G).
...
Proof.
elim => [| n Hrec] //.
  by rewrite expn1 -(card_sub H).
by rewrite expnS -{1}(card_sub H) -Hrec -card_prod.
Qed.
...
Lemma elt_prodnS:
  forall k n p, @elt_prodn k (S n) p =
               let (x, p1) := p in
               if k is S k1 then elt_prodn k1 p1 else (val x).
done.
Qed.
...
Lemma permSet1_def: forall n (H1: H 1),
  (codom (add_inv (n:=n)) (prodn_id (S n) H1)).
Proof.
move => n H1.
replace (prodn_id (S n) H1) with (add_inv (prodn_id n H1)).
  by apply codom_f.
...
have H1 := group1 group_H.
move => n; exists (prodn_id (S n) H1).
apply permSet1_def.
Defined.
...
apply: (etrans  (card_sub _)).
apply: (etrans (card_codom (@add_inv_inj n))).
by rewrite card_prodn.
Qed.

Let zp (n:nat):= @zp_group (S (S n)) is_true_true.
...
Lemma permuteS: forall n k (p: prodn n),
  permute (S k) p = permute1 (permute k p).
done.
Qed.
...
rewrite permuteS elt_add_permute1 // Rec.
 by rewrite -(addSnnS _ k2) addn1 -modn_add modn_mod -(modn_add (S k1)).
...
by rewrite elt_add_permute // addnC -{1}(mul1n (S _)) (modn_addl_mul 1)
        // modn_small.
Qed.
...
Lemma permute_zp_def:
forall n k u,
 k < S (S n) -> codom (add_inv (n:=n)) u ->
 (codom (add_inv (n:=n)) (permute (n:=S n) k u)).
move => n k u Hk Hu.
...
rewrite !elt_add_permute //.
    rewrite -modn_add modn_mod modn_add.
    by rewrite -[modn (_ + k2) _]modn_add modn_mod modn_add addnA.
by rewrite -ltnS ltn_mod.
Qed.
...
Proof.
move => n; rewrite /stab /SO /stabilizer.
rewrite -{1}(filter_setA (enum (zp n))).
apply/eqP; apply: eq_filter.
move => [x Hx]; symmetry; rewrite /setA; apply/eqP.
rewrite /gpermute_zp can_graph_of_fun
        /permute_zp /permSet1.
apply/val_eqP; apply/eqP.
by move: (permute_id_rev (S n) x (group1 group_H)).
Qed.
