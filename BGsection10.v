...
Local Notation ideal := (fun p =>
  (existsb P:{group _}, p.-Sylow(G) P && ((~~ p.-narrow P) || ('r_p(P) <= 2)))).
...
have thA : forall V, P ><| V = 'N(P) -> [~: P, V] = P /\ P \subset 'N(P)^`(1).
  move=> V defV; have Hall_P := pHall_Hall pHall_P.
  ...
  by case: (solvable_hall_dprod_der_subSset_comm_centr_compl _ _ defV sPNP').
...
split=> //.
  ...
  rewrite inE; case/andP; move/eqP=> TI_PV; move/eqP=> /= defPV rP.
  ...
  apply/eqP; rewrite defO1.
  rewrite eqEcard -nil_class2 (leq_trans (nil_class_pgroup pP1)) ?lcP //.
(*
  have ntP1 : P1 :!=: 1 by case: eqP lcP => // ->; rewrite cards1 logn1.
  have ntZP : 'Z(P1) != 1. 
    by apply: contra ntP1; move/eqP=> ?; rewrite (trivg_center_pgroup pP1).
  have ntP' : P1^`(1) != 1.
    apply: contra nAP; rewrite (isog_abelian (quotient1_isog _)); move/eqP=><-.
    by rewrite /= -{2}(derg0 P1) der_abelian.
  have := sol_der1_proper solP _  ntP1; rewrite -def_P mulG_subl.
  move/(_ (erefl _)); move/proper_card => pP'P1.
  have: 'Z(P1) \proper P1.
    rewrite properEneq subsetIl andbT; apply: contra nAP; move/eqP=> <-.
    exact: center_abelian.
  move/proper_card => pZP1.
  have pabZ : p.-abelem 'Z(P1).
    by rewrite abelemE ?center_abelian ?(dvdn_trans (exponentS (subsetIl _ _))).
  have pabP' : p.-abelem P1^`(1).
    rewrite abelemE ?(dvdn_trans (exponentS (der_sub _ _))) //.
    rewrite (p2group_abelian (pgroupS (der_sub _ _) pP1)) // -ltnS -lcP. 
    rewrite properG_ltn_log // (sol_der1_proper solP _  ntP1) //.
    by rewrite -def_P mulG_subl.
*)
  by admit. 
  ...
  rewrite (('N_P(Q) :=P: (S :^ (x * y^-1)) :^ t^-1) _); last first.
...
Let Hall_G_Mbeta : \beta(M).-Hall(M) M`_\beta -> \beta(M).-Hall(G) M`_\beta.
Proof.
...
have : forall p, prime p -> p \notin \beta(M) -> [/\
  \beta(M).-Hall(M) M`_\beta, \beta(M).-Hall(G) M`_\beta,
        p^'.-Hall(M^`(1)) 'O_p^'(M^`(1)), 
        p^'.-Hall(M`_\sigma) 'O_p^'(M`_\sigma) &
        forall q, prime q -> q %| #| M / 'O_p^'(M) | -> q <= p].
  move=> p p_pr n_pB; have [P pSyl_P] := Sylow_exists p M.
  have pl1M := mFT_proper_plength1 p_pr (mmax_proper M_max).
  ...
  case: (odd_narrow_plength1_complement_max_pdiv oddM solM pSyl_P)=>[|pH_M bip].
    move: (n_pB); rewrite !inE; case/nandP.
      by move/negbTE; rewrite (p_rank_Sylow pSyl_P)=> ->; exact: implyFb.
    apply: contraR; rewrite negb_imply pl1M; case/andP=> rP; case/nandP=> nP //.
    apply/existsP; exists P; rewrite nP (mmax_sigma_Sylow M_max pSyl_P) //.
    apply: uniq_mmax_norm_sub. (* unicity of M over P *)
    by admit.
  have sMBOM' : MB \subset 'O_p^'(M^`(1)).
    rewrite (subset_normal_Hall _ pH_M) /psubgroup ?pcore_normal //= p'MB andbT.
    by admit.
  have nI : M^`(1) \subset 'N('O_p^'(M`_\sigma) :&: 'O_p^'(M^`(1))).
    rewrite normsI // ?(char_norm (pcore_char p^' M^`(1))) //.
    by rewrite -pcoreI (subset_trans (der_sub 1 _)) // char_norm ?pcore_char.
  have cI : \beta(M).-subgroup(M^`(1)) ('O_p^'(M`_\sigma) :&: 'O_p^'(M^`(1))).
    rewrite /psubgroup subIset 1?orbC ?pcore_sub //=.
    by admit. (* ??? *)
  have sMBI : MB \subset ('O_p^'(M`_\sigma) :&: 'O_p^'(M^`(1))).
    rewrite subsetI sMBOM' andbT -pcoreI pcore_max //=.
      rewrite [_.-group _]pnatI [_.-nat _]p'MB. 
      apply: sub_in_pnat _ (pHall_pgroup bHall_MB) => x _ ?.
      by apply: alpha_sub_sigma M_max _ _; apply: beta_sub_alpha _ _.
    by admit. (* MB <| M *)
  have defMB : MB :=: 'O_p^'(M`_\sigma) :&: 'O_p^'(M^`(1)).
    apply/eqP; rewrite eqEsubset sMBI.
    have sOM'M : 'O_p^'(M^`(1)) \subset M.
      by apply: subset_trans (pcore_sub _ _) (der_sub _ _).
    by admit.
  have {defMB} defMB : MB :=: M`_\beta.
    have defMB': MB :=: 'O_\beta(M)(M^`(1)).
      rewrite defMB.
      by admit.
    have nMBM : MB <| M.
      by rewrite defMB' char_normal // (char_trans (pcore_char _ _) (der_char _ _)).
    apply/eqP; rewrite eqEsubset defMB' pcore_max ?pcore_pgroup /= -?defMB'//=.
    by rewrite (subset_normal_Hall _ bHall_MB) // /psubgroup pcore_sub pcore_pgroup.
  split=> //; rewrite defMB in bHall_MB => //; first by apply: Hall_G_Mbeta.
  by admit.
move=> thAC; split; last by move=> p pr_p n_pB; case: (thAC _ pr_p n_pB).
  have [p pr_p np_B] : exists2 p, prime p & p \notin \beta(M).
    by admit. (* Markov? fintype of \pi(M) + existsb? *)
  by case: (thAC _ pr_p np_B).
have nilQ : nilpotent (M^`(1) / M`_\beta).
  by admit. (* maybe bigdprod_nil *)
move=> H pHall_H.
have ti_HMb : H :&: M`_\beta :=: 1.
  apply: coprime_TIg; rewrite coprime_sym (pnat_coprime (pcore_pgroup _ _)) //.
  by case/orP: pHall_H; move/pHall_pgroup.
rewrite (isog_nil (quotient_isog _ ti_HMb)) ?(nilpotentS _ nilQ) ?quotientS //.
  case/orP: pHall_H; move/pHall_sub=> // sHMs; apply: subset_trans sHMs _. 
  exact: Msigma_sub_M'.
apply: subset_trans _ (char_norm (pcore_char _ _)).
case/orP: pHall_H; move/pHall_sub; move/subset_trans; apply; apply: char_sub;
  exact: bgFunc_char.
Admitted.
  
Corollary foo_10_9a : 
  forall p q : nat , prime p -> prime q
   -> p != q -> p \notin \beta(M)-> q \notin \beta(M)->
...
pose pq := [pred x | (x == p) || (x == q) ]; have solM := mmax_sol M_max.
have sXM'M : (X <*> M^`(1)) \subset M by rewrite mulgen_subG sXM der_sub.
have solXM' : solvable (X <*> M^`(1)) := solvableS sXM'M solM.
have pqX : pq.-group X.
  apply: sub_in_pnat _ qX => x _; move/(@eqP _ x q); rewrite inE /= => ->.
  by rewrite orbC eqxx.
...
have b'W :  \beta(M)^'.-group W.
  ...
  rewrite 3!inE /=; apply: contraL; rewrite negb_or => xB.
...
have nilW : nilpotent W.
  ...
  have qSyl_WO : q.-Sylow(W) ('O_p^'(M) :&: W).
    rewrite pHallE subsetIr /= eqn_dvd /=.
     have qXX : q.-group('O_p^'(M) :&: W).
    ...
    rewrite -(part_pnat_id qXX).
    rewrite (partn_dvd q (cardG_gt0 _) (cardSg (subsetIr _ _))).
    have E2 : #|M|`_q <= #|'O_p^'(M)|`_q.
      move/negbT: E; rewrite card_quotient ?char_norm ?pcore_char //.
      rewrite -divgS ?pcore_sub //=.
      apply: contraR; rewrite leqNgt; move/negbNE => E.
      have [k defk] := dvdnP (cardSg (pcore_sub p^' M)).
      rewrite defk mulnK ?cardG_gt0 //.
      have k_gt0 : 0 < k.
        by rewrite lt0n; case: eqP defk (cardG_gt0 M)=>// ->; rewrite mul0n=>->.
      rewrite defk partn_mul // -ltn_divl //= divnn part_gt0 p_part_gt1 in E.
      by rewrite inE /= mem_primes in E; case/and3P: E.
    have {E2} E2 : #|M|`_q == #|'O_p^'(M)|`_q.
      by rewrite eqn_leq E2 dvdn_leq // partn_dvd // cardSg // pcore_sub.
    by admit.
  ...
  have pSyl_OWM' : p.-Sylow(W) 'O_p(M^`(1) :&: W).
    by admit.    
  have nOWM'W : W \subset 'N('O_p(M^`(1) :&: W)).
    by admit.
  have defW : 'O_p(M^`(1) :&: W) \x ('O_p^'(M) :&: W) = W.
    by admit.
  rewrite (dprod_nil defW) (pgroup_nil (pcore_pgroup _ _)) //= => _.
  by rewrite (pgroup_nil (pHall_pgroup qSyl_WO)).
have pqHall_WMs : pq.-Hall(M`_\sigma) (W :&: M`_\sigma).
  apply: HallSubnormal solXM' _ pqHall_W.
  rewrite /normal (subset_trans (Msigma_sub_M' M_max)) ?mulgen_subr //=.
  by rewrite (subset_trans sXM'M) // (char_norm (pcore_char _ _)).
have sXCWMs : X \subset 'C(W :&: M`_\sigma).
  by admit.
split; first exists (W :&: M`_\sigma)%G => //.
  by admit.
  by admit.
move=> qSyl_X.
...
have nMbXM : M`_\beta <*> X <| M.
  ...
  rewrite ((M`_\beta <*> X :=P: 'O_bq(M^`(1))) _).
    exact: char_normal_trans (pcore_char _ _) (der_normal 1 _).
  by admit.
pose U := 'N_M(X).
...
have qSig : q \in \sigma(M).
  ...
  by exists S; rewrite sNSM (pHall_subl _ _ qSyl_S) ?subsetT // sSM.
have qB' : q \notin \beta(M).
  by admit.
...
have nNMSU : 'N_M(S)%G \notin 'U.
  apply: contra neq_HM => nMSU; apply/eqP; apply: group_inj. 
