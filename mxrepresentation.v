...
(* to the action of a group on an elementary abelian p-group.               *)
(*   The operations defined here include:                                   *)
(* 'A[F]_(m, n) == the type of matrices encoding (sub)algebras of square    *)
(*                 n x n matrices via mxvec; as in the corresponding matrix *)
(*                 notation, m and F can be omitted (m defaults to n ^ 2).  *)
(*              := 'M[F]_(m, n ^ 2)                                         *)
(* (A \in R)%MS <=> the square matrix A belongs to the linear set of        *)
(*                 matrices (most often, a sub-algebra) encoded by the row  *)
(*                 space of R. This is simply notation, so all the lemmas   *)
(*                 and rewrite rules fo (_ <+ _)%MS apply directly.         *)
(*              := (mxvec A <= R)%MS                                        *)
...
(* MatrixFormula.mxnonsimple_sat rG U == U is not a simple as an rG-module  *)
(*                 This is a bool predicate, which requires a decField      *)
(*                 structure on the scalar field. The MatrixFormula module, *)
(*                 described below, is a general toolkit for building       *)
(*                 formal expressions involving matrices.                   *)
...
(*  + submx_form A B == a GRing.formula asserting that the row space of     *)
...
Proof.
rewrite rmorph_sub -horner_mx_X -rmorphX /=.
...
by rewrite -rmorphM horner_mx_C -scalar_mx_add scalar_mx_is_scalar.
Qed.
...
Lemma rfix_mx_rstabC : forall (H : {set gT}) m (U : 'M[F]_(m, n)),
  H \subset G -> (H \subset rstab U) = (U <= rfix_mx H)%MS.
Proof.
...
move=> H; apply/eqmxP; apply/andP; split.
  ...
  by apply: (canRL (mulmxKV uB)); rewrite -rconj_mxJ mulmxK // rfix_mx_id.
...
move=> K sKG; apply/eqmxP; apply/andP; (split; apply/rfix_mxP) => [x Kx | Hx].
  have Gx := subsetP sKG x Kx.
  by rewrite -quo_mx_coset // rfix_mx_id // mem_morphim ?(subsetP nHG).
...
Lemma mx_faithful_irr_center_cyclic : forall n rG,
  @mx_faithful _ G n rG -> mx_irreducible rG -> cyclic 'Z(G).
Proof.
...
Lemma mx_faithful_irr_abelian_cyclic : forall n rG,
  @mx_faithful _ G n rG -> mx_irreducible rG -> abelian G -> cyclic G.
Proof.
...
move=> n rG U V modU modV; split=> [[B eqrUV injB homB] | [f injf homf defV]].
  ...
  case/complete_unitmx => f injf defUf; exists f => //.
    ...
    rewrite -in_submodJ //; have [u ->] := submxP (mxmoduleP modU x Gx).
...
Definition irr_repr i : mx_representation G (n_ i) := socle_repr i.
...
Variables (gT : finGroupType) (S : {group gT}) (p n : nat).
Hypotheses (pS : p.-group S) (esS : extraspecial S).
Hypothesis oSpn : #|S| = (p ^ n.*2.+1)%N.
Hypotheses (splitF : group_splitting_field S) (F'S : [char F]^'.-group S).
...
Variables (m : nat) (rS : mx_representation S m) (U : 'M[F]_m).
...
by rewrite mxE -(map_scalar_mx f) inj_eq //; exact: map_mx_inj.
Qed.
...
apply: (big_rel Rf) => [|P1 P1f P2 P2f ? ?|i _]; first by rewrite /Rf map_mx0.
  by apply: eqmx_trans (map_addsmx _ _ _) _; exact: adds_eqmx. 
...
Definition map_repr_mx g := (rG g)^f.

Lemma map_mx_repr : mx_repr G map_repr_mx.
Proof.
...
Lemma map_section_repr : forall n (rG : mx_representation aF G n) rGf U V,
  ...
  map_repr rG =1 rGf ->
  mx_rsim (map_repr (section_repr modU modV)) (section_repr modUf modVf).
Proof.
...
exact: map_section_repr map_regular_repr.
Qed.
...
by case: (x != 0).
Qed.
Let zRM := scalar_mx_rmorphism F n'.

Lemma mxval_inj : injective mxval.
Proof. exact: inj_comp (@horner_rVpoly_inj _ _ A) val_inj. Qed.

Let mxRM := horner_mx_rmorphism A.

Lemma mxval0 : mxval 0 = 0.
Proof. by rewrite /mxval [pval _]rVpoly0 rmorph0. Qed.

Lemma mxvalN : {morph mxval : x / - x}.
Proof. by move=> x; rewrite /mxval [pval _]rVpolyN rmorphN. Qed.

Lemma mxvalD : {morph mxval : x y / x + y}.
Proof. by move=> x y; rewrite /mxval [pval _]rVpolyD rmorphD. Qed.
...
by rewrite -horner_mxK mx_inv_hornerK ?horner_mx_mem // (rmorphM mxRM).
Qed.
...
move: F' (gen_rmorphism _ _ : {rmorphism F -> F'}) => F' f'.
set rG' := map_repr _ rG => irrG'; case: notF.
