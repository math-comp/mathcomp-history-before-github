...
move: dpq;  rewrite size_poly_eq0 in p0; rewrite /dvdp (eqP p0).
by rewrite modp0 (negbTE n0q).
Qed.

Lemma eqp_polyC: forall c : R, (c != 0) = (c%:P %= 1).
Proof.
...
Fixpoint gdcop_rec (q p : {poly R}) n :=
  if n is m.+1 then
