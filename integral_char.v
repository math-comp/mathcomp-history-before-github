...
Definition subadd_pred := 0 \in S /\ {in S &, forall u v, u - v \in S}.

Hypothesis addpS : subadd_pred.

Lemma raddp0 : 0 \in S. Proof. by case: addpS. Qed.
Lemma raddp_sub : {in S &, forall u v, u - v \in S}. Proof. by case: addpS. Qed.

Lemma raddpN u : (- u \in S) = (u \in S).
Proof. by apply/idP/idP=> /(raddp_sub raddp0); rewrite sub0r ?opprK. Qed.

Lemma raddpD : {in S &, forall u v, u + v \in S}.
Proof. by move=> u v Su Sv; rewrite -[v]opprK raddp_sub ?raddpN. Qed.

Lemma raddp_sum I r (P : pred I) F :
  (forall i, P i -> F i \in S) -> \sum_(i <- r | P i) F i \in S.
Proof. by move=> IH; elim/big_ind: _; [exact: raddp0 | exact: raddpD |]. Qed.

Lemma raddpMn n : {in S, forall u, u *+ n \in S}.
Proof. by move=> u Su; rewrite -(card_ord n) -sumr_const raddp_sum. Qed.
...
Definition submul_pred := 1 \in S /\ {in S &, forall u v, u * v \in S}.

Hypothesis mulpS : submul_pred.

Lemma rmulp1 : 1 \in S. Proof. by case: mulpS. Qed.
Lemma rmulpM : {in S &, forall u v, u * v \in S}. Proof. by case: mulpS. Qed.

Lemma rmulp_prod I r (P : pred I) F :
  (forall i, P i -> F i \in S) -> \prod_(i <- r | P i) F i \in S.
Proof. by move=> IH; elim/big_ind: _; [exact: rmulp1 | exact: rmulpM |]. Qed.

Lemma rmulpX n : {in S, forall u, u ^+ n \in S}.
Proof. by move=> u Su; rewrite -(card_ord n) -prodr_const rmulp_prod. Qed.

End MulPred.

Definition subring_pred := {in S &, forall u v, u - v \in S} /\ submul_pred.

Hypothesis ringpS : subring_pred.

Lemma ringp_mul : submul_pred. Proof. by case: ringpS. Qed.
Lemma ringp_add : subadd_pred S.
Proof.
by have [Ssub [S_1 _]] := ringpS; have:= Ssub _ _ S_1 S_1; rewrite subrr.
Qed.

Definition ringp0 := raddp0 ringp_add.
Definition ringpN := raddpN ringp_add.
Definition ringpD := raddpD ringp_add.
...
Definition ringpX := rmulpX ringp_mul.

Lemma ringp_nat n : n%:R \in S.
Proof. by rewrite ringpMn ?ringp1. Qed.

Lemma ringp_sign n : (-1) ^+ n \in S.
...
Lemma lead_coef_lreg (p : {poly R}) a :
  GRing.lreg a -> lead_coef (a *: p) = a * lead_coef p.
Proof. by move=> reg_a; rewrite !lead_coefE coefZ lreg_size. Qed.

Lemma subring_horner S :
   subring_pred S ->
   forall (p : {poly R}) x, all (mem S) p -> x \in S -> p.[x] \in S.
Proof.
move=> ringS p x /(all_nthP 0)/=Sp Sx; rewrite horner_coef ringp_sum // => i _.
by rewrite ringpM ?Sp ?ringpX.
Qed.

Lemma prod_monic I r (P : pred I) (F : I -> {poly R}):
  (forall i, P i -> monic (F i)) -> monic (\prod_(i <- r | P i) F i).
Proof.
by move=> monF; elim/big_ind: _ => // [|p q /monic_mull-> //]; exact: monic1.
Qed.

Lemma prod_factors_monic I r (P : pred I) (F : I -> R) :
  monic (\prod_(i <- r | P i) ('X - (F i)%:P)).
Proof. by apply: prod_monic => i _; exact: monic_factor. Qed.

Lemma size_prod_factors I r (F : I -> R) :
  size (\prod_(i <- r) ('X - (F i)%:P)) = (size r).+1.
Proof.
elim: r => [|i r /= <-]; rewrite ?big_nil ?size_poly1 // big_cons.
rewrite size_monic_mul ?monic_factor ?monic_neq0 ?prod_factors_monic //.
...
Lemma cyclotomic_monic z n : monic (cyclotomic z n).
Proof. exact: prod_factors_monic. Qed.

Lemma size_cyclotomic z n : size (cyclotomic z n) = (phi n).+1.
Proof.
rewrite /cyclotomic -big_filter filter_index_enum size_prod_factors; congr _.+1.
rewrite -cardE -sum1_card phi_count_coprime big_mkord.
by apply: eq_bigl => k; rewrite coprime_sym.
Qed.

Section MorePrimRoot.
...
Lemma dvdn_prim_root d : (d %| n)%N -> d.-primitive_root (z ^+ (n %/ d)).
Proof.
set k := (n %/ d)%N => d_dv_n; rewrite -{1}(mulKn d n_gt0) -divn_divr // -/k.
by rewrite -{1}(@dvdn_gcd_idl k n) ?exp_prim_root // -(divnK d_dv_n) dvdn_mulr.
Qed.
...
Section CombinatorialMap.

Variable f : aR -> rR.

Lemma map_poly0 : map_poly f 0 = 0.
Proof. by rewrite map_polyE polyseq0. Qed.

Hypotheses (inj_f : injective f) (f_0 : f 0 = 0).

Lemma size_map_inj_poly p : size (map_poly f p) = size p.
Proof.
have [-> | nz_p] := eqVneq p 0; first by rewrite map_poly0 !size_poly0.
by rewrite size_poly_eq // -f_0 (inj_eq inj_f) -lead_coefE lead_coef_eq0.
Qed.

Lemma map_inj_poly : injective (map_poly f).
Proof.
move=> p q /polyP eq_pq; apply/polyP=> i; apply: inj_f.
by rewrite -!coef_map_id0_poly ?eq_pq.
Qed.

Lemma lead_coef_map_inj p : lead_coef (map_poly f p) = f (lead_coef p).
Proof. by rewrite !lead_coefE size_map_inj_poly coef_map_id0_poly. Qed.

End CombinatorialMap.

Lemma map_poly_comp (f : {rmorphism aR -> rR}) p q :
  map_poly f (p \Po q) = map_poly f p \Po map_poly f q.
Proof.
elim/poly_ind: p => [|p a IHp]; first by rewrite !raddf0.
rewrite comp_poly_amulX !rmorphD !rmorphM /= !map_polyC map_polyX.
by rewrite comp_poly_amulX -IHp.
Qed.

End MapRing.

Lemma lead_coefZ (R : idomainType) (p : {poly R}) a :
  lead_coef (a *: p) = a * lead_coef p.
Proof.
have [-> | /mulfI/lead_coef_lreg//] := eqVneq a 0.
by rewrite mul0r scale0r lead_coef0.
Qed.
...
rewrite -(prod_factors_of_unity prim_z) big_mkord.
...
Proof.
apply: sig_eqW; elim: {p}_.+1 {-2}p (ltnSn (size p)) => // n IHn p lePn.
...
set a := lead_coef p; pose p1 i := - p`_i / a.
have nz_a: a != 0 by rewrite lead_coef_eq0.
have p1_gt0: (0 < (size p).-1)%N by rewrite -subn1 subn_gt0.
...
Lemma modp_factor p (c : R): p %% ('X - c%:P) = p.[c]%:P.
Proof.
have: root (p - p.[c]%:P) c by rewrite /root !horner_lin subrr.
case/factor_theorem=> q /(canRL (subrK _))/unit.modpP-> //.
  by rewrite lead_coef_factor unitr1.
by rewrite size_polyC size_factor ltnS leq_b1.
Qed.

Lemma coprimep_factor p (c : R): coprimep p ('X - c%:P) = ~~ root p c.
Proof.
rewrite -coprimep_modl modp_factor /root -scale_poly1.
have [-> | /coprimep_scalel->] := altP eqP; last exact: coprime1p.
by rewrite scale0r /coprimep gcd0p size_factor.
Qed.

Lemma dvdp_mul_factor p q (c : R):
  (p %| ('X - c%:P) * q) = ((if root p c then p %/ ('X - c%:P) else p) %| q).
Proof.
case: ifPn => [| not_pc0]; last by rewrite gausspr ?coprimep_factor.
rewrite root_factor_theorem -eqp_div_factor mulrC => /eqP{1}->.
by rewrite dvdp_mul2l ?factor_eq0.
Qed.

Lemma dvdp_prod_factors I r (F : I -> R) p :
    p %| \prod_(i <- r) ('X - (F i)%:P) ->
  {m | p %= \prod_(i <- mask m r) ('X - (F i)%:P)}.
Proof.
elim: r => [|i r IHr] in p *.
  by rewrite big_nil dvdp1; exists nil; rewrite // big_nil -size_poly_eq1.
rewrite big_cons dvdp_mul_factor root_factor_theorem -eqp_div_factor.
case: eqP => [{2}-> | _] /IHr[m Dp]; last by exists (false :: m).
by exists (true :: m); rewrite /= mulrC big_cons eqp_mul2l ?factor_eq0.
Qed.
...
Lemma raddpMz M S :
  @subadd_pred M S -> forall z, {in S, forall u, u *~ z \in S}.
Proof. by move=> raddpS [n|n] u Su; rewrite ?raddpN ?raddpMn. Qed.

Definition ringpMz R S ringpS := raddpMz (@ringp_add R S ringpS).

Lemma ringp_int R S : @subring_pred R S -> forall z, z%:~R \in S.
Proof. by move=> ringpS z; rewrite ringpMz ?ringp1. Qed.
...
Lemma Qint_subring : subring_pred Qint.
Proof.
do 2?[split] => // _ _ /QintP[x ->] /QintP[y ->]; apply/QintP.
  by rewrite -rmorph_sub; exists (x - y).
by rewrite -rmorphM; exists (x * y).
Qed.
...
- rewrite [d]zintEsign d1 mulr1 negz_int_poly_scale lead_coef_Imul.
...
rewrite [p]int_polyEscale -/d [d]zintEsign mulrC -scalerA map_poly_scaler /=.
...
by rewrite {1 3}[p]int_polyEscale scale_poly_eq0 => /norP[/size_scaler-> _].
Qed.

Lemma int_scale_monic p : monic p -> int_poly_scale p = 1.
Proof.
...
by rewrite mulrC -scalerA -map_poly_scaler -int_polyEscale.
Qed.
...
have: root (\prod_(z <- [:: algCi; -algCi]) ('X - z%:P)) algCi^*.
  ...
  by rewrite /root !horner_lin -expr2 -rmorphX sqr_algCi rmorphN rmorph1 subrr.
by rewrite root_prod_factors !inE [_ == _](negPf algCi_nonReal) => /eqP.
Qed.
...
by rewrite -root_prod_factors -(root_scaler _ _ nz_a) -Dp.
Qed.
...
Lemma ringp_Cnat S : subring_pred S -> (forall x, isNatC x -> x \in S).
Proof. by move/ringp_nat=> S_n _ /isNatCP[n ->]. Qed.

Lemma ringp_Cint S : subring_pred S -> (forall x, isIntC x -> x \in S).
Proof. by move/ringp_int=> S_z x /getCintK <-. Qed.
...
rewrite coef_map coef_map_id0_poly //= -[p]coefK coef_poly.
...
Lemma Crat_subring : subring_pred Crat.
Proof.
do 2?[split] => // _ _ /CratP[x ->] /CratP[y ->].
  by rewrite -rmorph_sub ratr_Crat.
by rewrite -rmorphM ratr_Crat.
Qed.
Hint Resolve Crat_subring.

Lemma CratV x : (x^-1 \in Crat) = (x \in Crat).
Proof.
wlog suff: x / x \in Crat -> x^-1 \in Crat.
  by move=> IH; apply/idP/idP=> /IH //; rewrite invrK.
by case/CratP=> y ->; rewrite -fmorphV ratr_Crat.
Qed.

Lemma CratXz m : {in Crat, forall x, x ^ m \in Crat}.
Proof. by case: m => m x Qx; rewrite ?CratV ?ringpX. Qed.

Lemma Crat_div : {in Crat &, forall x y, x / y \in Crat}.
Proof. by move=> x y Qx Qy; rewrite /= ringpM ?CratV. Qed.
...
Fact minCpoly_subproof (x : algC) :
  {p | monic p & forall q, root (pQtoC q) x = (p %| q)}.
Proof.
...
have r_x: x \in r by rewrite Dp0 root_scaler // root_prod_factors in p0x.
...
have{d0 p0 nz_d0 p0x Dp0} /minset_exists[I /minsetP[]]: Qpx setT.
  rewrite /Qpx; have ->: p_ setT = d0^-1 *: intrp p0.
   ...
   rewrite root_scaler ?invr_eq0 // p0x; apply/(all_nthP 0)=> i _ /=.
   by rewrite coefZ mulrC coef_map Crat_div ?ringp_int.
case/andP=> pIx QpI minI _; pose p := map_poly CtoQ (p_ I).
have DpI: p_ I = pQtoC p.
  rewrite -[p_ I]coefK; apply/polyP=> i; rewrite -map_comp_poly !coef_poly.
  by case: ifP => //= lti_pI; rewrite getCratK //; exact: (all_nthP 0 QpI).
exists p; first by rewrite -(map_monic QtoC_M) -DpI prod_factors_monic.
move=> q; rewrite -(dvdp_map QtoC_M) -DpI.
apply/idP/idP=> [qx0 | /dvdpP[{q} q ->]]; last by rewrite root_mul pIx orbT.
pose q1 := gcdp p q; have /dvdp_prod_factors[m Dq1]: pQtoC q1 %| p_ I.
...
- exists (x *+ n + z).
  ...
  by rewrite map_poly_comp horner_comp_poly -Dpz.
...
by exists (n, [:: px; - pz]); rewrite /= !raddfN horner_opp -[z]opprK Dpz Dpx.
Qed.

Lemma num_field_exists (s : seq algC) :
  {Qs : fieldExtType qnum & {QsC : {rmorphism Qs -> algC}
   & {s1 : seq Qs |  map QsC s1 = s
   & forall E : {algebra Qs}, {subset s1 <= E} -> E = fullv _ :> {vspace Qs}}}}.
Proof.
...
- set inQs := fieldExt_horner z1 in z1gen *; pose s1 := map inQs ps.
  ...
  exists Qs, QsC, s1 => [|A As1].
    by rewrite -map_comp Ds (eq_map inQsK).
  ...
  suffices ->: z1 = \sum_(i < size s) s1`_i *+ a i.
    ...
    by rewrite memvMn ?As1 ?mem_nth // size_map sz_ps.
...
rewrite -horner_map -map_comp_poly [rhs in _ = rhs]subfx_inj_eval //.
congr _.[_]; last by rewrite /= subfx_inj_eval // map_polyX hornerX.
apply: eq_map_poly; exact: subfx_inj_base.
Qed.

Definition in_ratC_span s x :=
  exists a : {ffun 'I_(size s) -> qnum}, x = \sum_i QtoC (a i) * s`_i.

Fact ratC_span_subproof s x : {in_ratC_span s x} + {~ in_ratC_span s x}.
Proof.
...
Definition ratC_span s : pred algC := ratC_span_subproof s.
Lemma ratC_spanP s x : reflect (in_ratC_span s x) (x \in ratC_span s).
Proof. exact: sumboolP. Qed.

Lemma mem_ratC_span s : {subset s <= ratC_span s}.
Proof.
move=> _ /(nthP 0)[ix ltxs <-]; pose i0 := Ordinal ltxs.
apply/ratC_spanP; exists [ffun i => (i == i0)%:R].
rewrite (bigD1 i0) //= ffunE eqxx // rmorph1 mul1r.
by rewrite big1 ?addr0 // => i; rewrite ffunE rmorph_nat mulr_natl => /negbTE->.
Qed.

Lemma ratC_span_subadd s : subadd_pred (ratC_span s).
Proof.
split=> [|_ _ /ratC_spanP[x ->] /ratC_spanP[y ->]]; apply/ratC_spanP.
  by exists 0; rewrite big1 // => i; rewrite ffunE rmorph0 mul0r.
exists (x - y); rewrite -sumr_sub; apply: eq_bigr => i _.
by rewrite -mulr_subl -rmorph_sub !ffunE.
Qed.
Local Hint Resolve ratC_span_subadd.

(* Automorphism extensions. *)

(* Some of the internal facts need to be generalized to make the Galois       *)
(* theory usable.                                                             *)
...
pose subAut := {Qr : _ & numF_inj Qr * {rmorphism Qr -> Qr}}%type.
...
have ext1 mu0 x: {mu1 | exists xx, x = Sinj mu1 xx
...
- pose b0 := vbasis (fullv (Sdom mu0)).
  ...
  have{pz0} rz: z \in r by rewrite -root_prod_factors -Dr.
  have [Qr [QrC [rr Drr memQr]]] := num_field_exists r.
  ...
  pose im_in0 (E : {vspace Qr}) y := reflect (exists yy, y = in01 yy) (y \in E).
  have [K memK]: {K : {algebra Qr} & forall y, im_in0 K y}.
    suffices [K memK]: {K : {vspace Qr} & forall y, im_in0 K y}.
      apply: (existS _ (ASpace _) memK); rewrite has_aunit1.
        apply/prodvP=> _ _ /memK[y1 ->] /memK[y2 ->].
        by apply/memK; exists (y1 * y2); rewrite rmorphM.
      by apply/memK; exists 1; rewrite rmorph1.
    exists (span (map in01 b0)) => y; apply: (iffP idP)=> [b0y | [yy ->]].
      exists (\sum_i (coord [tuple of map in01 b0] y i) *: b0`_i).
      rewrite {1}(coord_span b0y) /= linear_sum; apply/eq_bigr=> i _.
      by rewrite /= linearZ (nth_map 0) ?size_tuple.
    rewrite (coord_basis (memvf yy)) linear_sum memv_suml // => i _.
    by rewrite linearZ memvZl ?memv_span ?map_f ?mem_nth ?size_tuple.
  suffices [E [f hom_f aut_f] [_  /allP srE]]: {E : {algebra Qr} &
      {f | kHom (aspace1 _) E f & {morph in01: y / Saut mu0 y >-> f y}}
    & (K <= E)%VS /\ all (mem E) rr}.
  + do [rewrite (memQr E) //; move/LAut_lrmorph] in hom_f.
    by exists (SubAut Qr QrC (RMorphism hom_f)); last exists in01.
  have{p r Dr Drr} Qrr: polyOver (aspace1 Qr) (\prod_(z <- rr) ('X - z%:P)).
    suffices ->: \prod_(z <- rr) ('X - z%:P) = map_poly (in_alg Qr) p.
      by apply/polyOverP=> i; rewrite coef_map memvZl ?mem1v.
    apply: (@map_poly_inj _ _ QrC).
    rewrite Sinj_poly Dr -Drr big_map rmorph_prod.
    by apply: eq_bigr => z _; rewrite rmorph_sub /= map_polyX map_polyC.
  have: all (mem rr) rr by exact/allP.
  elim: {-1}rr => [|z r IHr] /=.
    exists K => //.
    pose f y := in01 (Saut mu0 (sval (sig_eqW (memK _ (memv_proj K y))))).
    have lin_f: linear f.
      rewrite /f => a y1 y2; case: {1}(sig_eqW _) => z Dy /=.
      case: {1}(sig_eqW _) => z1 Dy1 /=; case: (sig_eqW _) => z2 Dy2 /=.
      rewrite linearP /= Dy1 Dy2 -linearP in Dy; rewrite -(fmorph_inj _ Dy).
      rewrite -[z1]mul1r scaler_mull !(rmorphM, rmorphD) scaler_mull.
      congr (_ * _ + _); apply: (fmorph_inj QrC).
      rewrite [QrC _](SinjZ _ [rmorphism of QrC \o in01 \o _]) SinjZ.
      by rewrite /= !rmorph1.
    exists (lapp_of_fun f) => [|z].
      apply/kHomP; split=> [_ /injvP[a ->]| y1 y2 Ky1 Ky2].
        rewrite linearZ //= => _ _; rewrite lapp_of_funK //.
        rewrite /f; case: (sig_eqW _) => z1 Dz1 /=.
        rewrite projv_id ?mem1v // -(rmorph1 in01) in Dz1.
        by rewrite -(fmorph_inj _ Dz1) !rmorph1.
      rewrite !lapp_of_funK // /f; case: {1}(sig_eqW _) => z Dy /=.
      case: {1}(sig_eqW _) => z1 Dy1 /=; case: (sig_eqW _) => z2 Dy2 /=.
      rewrite !projv_id ?memv_mul // in Dy1 Dy2 Dy.
      by rewrite Dy1 Dy2 -rmorphM in Dy; rewrite -!rmorphM (fmorph_inj _ Dy).
    rewrite lapp_of_funK // /f; case: (sig_eqW _) => z1 Dy /=.
    rewrite projv_id in Dy; last by apply/memK; exists z.
    by rewrite (fmorph_inj in01 Dy).
  case/andP=> rr_z /IHr[E [f hom_f Df] [sKE Er]] {IHr}.
  exists (Fadjoin_aspace E z); last first.
    rewrite memx_Fadjoin (subv_trans sKE) ?subsetKFadjoin //.
    by split=> //; apply: sub_all Er; exact: memK_Fadjoin.
  pose p := map_poly f (minPoly E z).
  have [z1 pz10]: {z1 | root p z1}.
    pose pr := \prod_(z <- rr) ('X - z%:P); have: p %| pr.
      have [p1 Dp] := polyOver_suba E _ (minPolyOver E z).
      pose f1 := RMorphism (kHomRmorph_subproof hom_f).
      have ->: p = map_poly f1 p1 by rewrite map_comp_poly -Dp.
      have Epr: polyOver E pr by apply: polyOver_subset Qrr; exact: sub1v.
      have [pr1 Dpr] := polyOver_suba E _ Epr; have ->: pr = map_poly f1 pr1.
        by rewrite  map_comp_poly -Dpr (kHomFixedPoly hom_f).
      rewrite dvdp_map -(dvdp_map [rmorphism of @sa_val _ _ E]) -Dp -Dpr.
      by rewrite minPoly_dvdp ?root_prod_factors.
    rewrite {}/pr; elim: (rr) => [|z1 r1 IHr1].
      rewrite big_nil dvdp1 size_poly_eq -?lead_coefE ?size_minPoly //.
      rewrite (monicP (monic_minPoly _ _)).
      by have /kHomP[fQ _] := hom_f; rewrite fQ ?oner_eq0 ?mem1v.
    by rewrite big_cons dvdp_mul_factor; case: ifP; first exists z1.
  exists (kHomExtend E f z z1) => [|y].
    apply: kHomExtendkHom => //; exact: sub1v.
  by rewrite kHomExtendExt ?Df // (subvP sKE) //; apply/memK; exists y.
pose fix ext n :=
  if n is i.+1 then oapp (fun x => s2val (ext1 (ext i) x)) (ext i) (unpickle i)
  else SubAut Qs QsC phi.
...
have rn1: all n.-unity_root r by apply/allP=> z; rewrite -root_prod_factors -Dp.
have sz_r: (n < (size r).+1)%N.
  by rewrite -(size_prod_factors r id) -Dp size_Xn_sub_1.
have [|z] := hasP (has_prim_root n_gt0 rn1 _ sz_r); last by exists z.
by rewrite -separable_factors -Dp separable_Xn_sub_1 // -neq0N_neqC -lt0n.
Qed.
...
Lemma Cyclotomic_monic n : monic 'Phi_n.
Proof.
rewrite /'Phi_n; case: (C_prim_root_exists _) => z /= _.
rewrite /monic lead_coefE coef_map_id0_poly ?(int_algC_K 0) //.
...
have mon_q: monic q by apply: prod_monic => d _; exact: Cyclotomic_monic.
...
have [r def_zn]: exists r, cyclotomic z n = pZtoC r.
  ...
  have /dvdpP[r0 Dr0]: map_poly ZtoQ q %| 'X^n - 1.
    rewrite -(dvdp_map QtoC_M) mapXn1 -map_comp_poly.
...
rewrite size_prod_factors -cardE phi_count_coprime big_mkord -sum1_card.
...
have mon_pz: monic pz by exact: cyclotomic_monic.
have pz0: root pz z.
  have [|n_gt1|n1] := ltngtP n 1; first by rewrite ltnNge n_gt0.
    rewrite [pz](bigD1 (Ordinal n_gt1)) ?coprime1n //=.
    by rewrite root_mul root_factor eqxx.
  rewrite /pz n1 [_ z _]big_ord1_cond root_factor expr0.
...
have /dvdpP_rat_int[f [af nz_af Df] [g /esym Dfg]]: pf %| pZtoQ 'Phi_n.
  rewrite -dv_pf; congr (root _ z): pz0; rewrite -Dpz -map_comp_poly.
  by apply: eq_map_poly => b; rewrite /= rmorph_zint.
without loss{nz_af} [mon_f mon_g]: af f g Df Dfg / monic f /\ monic g.
...
have{af Df} Df: pQtoC pf = pZtoC f.
  ...
  rewrite !(monicP _) // mulr1 Df => <-; rewrite scale1r -map_comp_poly.
  by apply: eq_map_poly => b; rewrite /= rmorph_zint.
have [/size1_polyC Dg | g_gt1] := leqP (size g) 1.
  rewrite /monic Dg lead_coefC in mon_g.
...
have [zk gzk0]: exists zk, root (pZtoC g) zk.
  ...
  rewrite size_prod_factors in g_gt1.
  by exists rg`_0; rewrite Dg root_prod_factors mem_nth.
have [k cokn Dzk]: exists2 k, coprime k n & zk = z ^+ k.
  have: root pz zk by rewrite -Dpz -Dfg rmorphM root_mul gzk0 orbT.
...
have [|k_gt1] := leqP k 1; last have [p p_pr /dvdnP[k1 Dk]] := pdivP k_gt1.
  rewrite -[leq k 1](mem_iota 0 2) !inE => /pred2P[k0 | ->]; last first.
    by rewrite -Df dv_pf.
  have /eqP := size_Cyclotomic n; rewrite -Dfg size_mul_monic ?monic_neq0 //.
...
rewrite -[g]comp_polyX map_poly_comp -horner_map /= Frobenius_autE -rmorphX.
rewrite -!map_comp_poly (@eq_map_poly _ _ _ (polyC \o *~%R 1)); last first.
  by move=> a; rewrite /= !rmorph_zint.
rewrite map_comp_poly -[_.[_]]map_poly_comp /= => co_fg.
suffices: coprimep (pZtoC f) (pZtoC (g \Po 'X^p)).
  ...
  rewrite -Dpz -Dfg rmorphM root_mul => /orP[] //= /IHm-> //.
...
have d_dv_mon h: d %| h -> monic h -> exists h1, h = d1 * h1.
...
have pzn_zk0: root (map_poly \1%VS (minPoly Q1 zn)) (zn ^+ k).
  rewrite -(fmorph_root QnC) rmorphX Dz -map_comp_poly.
  rewrite (@eq_map_poly _ _ _ QnC) => [|a]; last by rewrite /= unit_lappE.
  set p1 := map_poly _ _.
  have [q1 Dp1]: exists q1, p1 = pQtoC q1.
    have /polyOverP/(_ _)/injvP/sig_eqW a_ := minPolyOver Q1 zn.
  ...
  case/dvdpP=> r1 ->; rewrite rmorphM root_mul /= -Dq2; apply/orP; right.
  rewrite (minCpoly_cyclotomic prim_z) /cyclotomic.
  rewrite (bigD1 (Ordinal (ltn_pmod k n_gt0))) ?coprime_modl //=.
  by rewrite root_mul root_factor prim_expr_mod ?eqxx.
have phiM: lrmorphism phi.
  apply/LAut_lrmorph.
  rewrite -(genQn (Fadjoin_aspace (aspace1 _) zn)) ?kHomExtendkHom //.
  by apply/allP; rewrite /= memx_Fadjoin.
...
Lemma dec_intQ_span (V : vectType qnum) m (s : m.-tuple V) v :
...
Lemma dec_intC_span (V : vectType algC) m (s : m.-tuple V) v :
  {inIntSpan s v} + {~ inIntSpan s v}.
Proof.
have s_s (i : 'I_m): s`_i \in span s by rewrite memv_span ?mem_nth ?size_tuple.
have s_Zs a: \sum_(i < m) s`_i *~ a i \in span s.
...
pose inQzs w := forallb j, ratC_span z_s (coord b w j).
...
have xv j: {x | coord b v j = QzC x}.
  apply: sig_eqW; have /ratC_spanP[x ->] := forallP Qz_v j.
...
Definition intC_span (s : seq algC) : pred algC :=
  fun x => dec_intC_span (in_tuple [seq \row_(i < 1) y | y <- s]) (\row_i x).

Lemma intC_spanP n (s : n.-tuple algC) x :
  reflect (inIntSpan s x) (x \in intC_span s).
Proof.
...
Lemma root_monic_algInt p x :
  root p x -> monic p -> all isIntC p -> x \in algInt.
Proof.
have pZtoQtoC pz: pQtoC (pZtoQ pz) = pZtoC pz.
  by rewrite -map_comp_poly; apply: eq_map_poly => b; rewrite /= rmorph_zint.
...
suffices ->: p = ('X - a%:P) by rewrite seq_factor /= rmorphN isIntC_opp.
have /irredp_factor: p %| 'X - a%:P by rewrite -dv_p fmorph_root root_factor.
rewrite -size_poly_eq1 -(size_map_poly QtoC_M) -Dp.
by rewrite eqn_leq leqNgt size_minCpoly eqpMP ?monic_factor //= => /eqP.
Qed.

Lemma algInt_Cint x : isIntC x -> x \in algInt.
Proof.
move=> Zx; apply: root_monic_algInt (monic_factor x) _.
  by rewrite root_factor eqxx.
by rewrite seq_factor /= isIntC_opp Zx isIntC_1.
Qed.
...
apply/(all_nthP 0)=> i _; rewrite coef_sub coefC -mulrb coefXn.
...
have S_P := intC_spanP [tuple of Y]; set S := intC_span _ in S_P.
have Sadd: subadd_pred S by exact: intC_span_subadd.
have sYS: {subset Y <= S} by exact: mem_intC_span.
have S_1: 1 \in S.
  by apply/sYS/imageP; exists 0 => //; rewrite big1 // => i; rewrite ffunE.
have SmulX (i : 'I_m): {in S, forall x, x * X`_i \in S}.
  move=> _ /S_P[x ->]; rewrite -mulr_suml raddp_sum // => j _.
  rewrite mulrzAl raddpMz {x}// nth_image mulrC (bigD1 i) //= mulrA -exprS.
  ...
  rewrite -/(n i) Dp mulNr raddpN // -mulr_suml raddp_sum // => [[e le_e]] /= _.
  rewrite coef_map -mulrA mulrzl raddpMz ?sYS //; apply/imageP.
...
do 2?[split] => // x y Sx Sy; first by rewrite raddp_sub.
case/S_P: Sy => {y}[y ->]; rewrite -mulr_sumr raddp_sum //= => j.
rewrite mulrzAr raddpMz {y}// nth_image; move: {j}(enum_val j) => f.
...
Theorem fin_Csubring_algInt S n (Y : n.-tuple algC) :
    subring_pred S -> (forall x, reflect (inIntSpan Y x) (x \in S)) ->
  {subset S <= algInt}.
Proof.
pose ZP := [pred p : {poly algC} | all isIntC p].
...
have ZP_1: 1 \in ZP by rewrite inE /= polyseq1 /= isIntC_1.
have ZP_X: 'X \in ZP by rewrite inE /= polyseqX /= isIntC_1 isIntC_0.
have ringZP: subring_pred ZP.
  do 2?[split]=> // p q /allcP Zp /allcP Zq; apply/allcP=> i.
    by rewrite coef_sub isIntC_sub ?Zp ?Zq.
  by rewrite coefM isIntC_sum // => j _; rewrite isIntC_mul ?Zp ?Zq.
have ZP_C c: (ZtoC c)%:P \in ZP by rewrite raddfMz ringp_int.
move=> ringS S_P x Sx; pose v := \row_(i < n) Y`_i.
...
have sYS (i : 'I_n): x * Y`_i \in S.
  rewrite ringpM //; apply/S_P/intC_spanP/mem_intC_span.
  by rewrite -tnth_nth mem_tnth.
...
Corollary algInt_subring : subring_pred algInt.
Proof.
suff rAZ: {in algInt &, forall x y, (x - y \in algInt) * (x * y \in algInt)}.
  by do 2?[split] => // x y AZx AZy; rewrite rAZ.
move=> x y AZx AZy.
...
rewrite ringp_sum // => i _; apply: (@algInt_unity_root #[x]) => //.
exact/unity_rootP.
Qed.
...
have S_P := intC_spanP X; set S := intC_span X in S_P.
have Sadd: subadd_pred S by exact: intC_span_subadd.
have S_X: {subset X <= S} by exact: mem_intC_span.
have S_1: 1 \in S.
...
have Smul: {in S &, forall x y, x * y \in S}.
  move=> _ _ /S_P[x ->] /S_P[y ->]; rewrite -mulr_sumr raddp_sum // => j _.
  rewrite mulrzAr -mulr_suml raddpMz ?raddp_sum // => k _.
  rewrite mulrzAl raddpMz {x y}// !nth_mktuple.
  rewrite -gring_irr_modeM ?gring_class_sum_central //.
  rewrite gring_classM_expansion raddf_sum raddp_sum // => jk _.
  by rewrite scaler_nat raddfMn raddpMn ?S_X ?memX.
by move=> k; apply: fin_Csubring_algInt S_P _ _; rewrite ?S_X //; case: Sadd.
Qed.
...
Theorem dvd_irr1_cardG : dvdC ('chi[G]_i 1%g) #|G|%:R.
Proof.
...
Theorem dvd_irr1_index_center (gT : finGroupType) (G : {group gT}) i :
