...
(* [pred x | E]            == predicate x => E                               *)
(* [pred x : T| E]         == predicate x => T, with a cast on the argument  *)
(* [pred x \in A]          == transforms a collective predicate A into an    *)
(*                            applicative one                                *)
(*                                                                           *)
(* [pred x \in A | E]      == predicate x => (x \in A) && E                  *)
(* x \in A                 == infix syntax for 'x satisfies the              *)
(*                            (collective) pred  A'                          *)
(*                                                                           *)
(* x \notin A              == infix syntax for 'x does not satisfies the     *)
(*                            (collective) predicate A'                      *)
(* [predU A & B]           ==  union of two  predicates. Variants with       *)
(*                            intersection (predI), difference (predD)       *)
(*                            complement (predC)                             *)
(*                                                                           *)
(* [preim f of A]          == preimage by f of the collective predicate A    *)
(* [rel x y | E]           == relation                                       *)
(* [rel x y : T | E]       == relation, with a cast on the arguments         *)
(* [rel x y \in A & B | E] == relation, x in the collective predicate        *)
(*                             A and y in the collective predicate B         *)
(* [rel x y \in A]         == membership relation of both arguments to the   *)
(*                            collective predicate A                         *)
(*                                                                           *)
(* Localization of (Prop) predicates:                                        *)
(*                                                                           *)
(* {in d , P}             == forall x, x \in d -> P x                        *)
(* {in d1 & d2 , P}       == forall x y, x \in d1 -> y \in d2 -> P x y       *)
(*                            Variant for ternary predicates                 *)
(* {in d & , P}           == forall x y, x \in d -> y \in d -> P x y         *)
(* {in d1 & d2 &, P}      == forall x y z,                                   *)
(*                            x \in d1 -> y \in d2 -> z \in d2 -> P x y z    *)
(*                            + Variants                                     *)
(* {in d, bijective f}    == f has a right inverse on d                      *)
(* {on cd, P}             == forall x, (f x) \in cd -> P x                   *)
(*                            + Variants à la {in ...}                       *)
(*                                                                           *)
(*                                                                           *)
