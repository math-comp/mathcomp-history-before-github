...
let dC c = CastConv (DEFAULTcast, c)
...
let brain_dead_fix = function
  | ImplicitBindings xs -> ImplicitBindings (List.map snd xs)
  | ExplicitBindings xs -> ExplicitBindings (List.map (fun (l,q,x) -> (l,q,snd x)) xs)
...
TACTIC EXTEND ssrrewrite
  ...
  | [ "rewrite" ssrtermspc(r) "with" bindings(bl) ssrclauses(clauses) ] ->
    [ tclCLAUSES (ssrrewritebindtac r (brain_dead_fix bl) wild_guess) clauses ]
...
let rec format_rawdecl h0 d0 = match h0, d0 with
  ...
  | BFdef true:: h, (x, Some (RCast (_, v, CastConv(_, t))), _) :: d ->
