...
Require Import commutators center cyclic gprod pgroups nilpotent sylow maximal.
Require Import transfer hall coprime_act mxrepresentation.
...
(* This file currently covers B & G 1.3-4, 1.6, 1.8-1.19, 1.21 as well as     *)
(* Aschbacher 23.7 and 24.7, and Gorenstein 8.1.3.                            *)
...
Definition stable_series (A : {set gT}) (s : seq {group gT}) :=
  path (stable_factor A) 1%G s.
...
(* B & G Proposition 1.5 is covered by several lemmas in coprime_act.v :      *)
...
Lemma coprime_abel_cent_TI : forall gT (A G : {group gT}),
...
(* B & G, Proposition 1.9 (internal coprime action) *)
...
Lemma comm_cent_cent_norm : forall gT (A G H : {group gT}),
    A \subset 'N(G) -> A \subset 'C(H) -> G \subset 'N(H) ->
  [~: G, A] \subset 'C(H).
Proof.
move=> gT A G H nGA; move/centsP=> cHA nHG; rewrite commGC gen_subG centsC.
...
Theorem abelian_charsimple_special : forall gT p (A G : {group gT}),
    p.-group G -> coprime #|G| #|A| -> [~: G, A] = G ->
    \bigcup_(H : {group gT} | (H \char G) && abelian H) H \subset 'C(A) ->
  special G /\ 'C_G(A) = 'Z(G).
Proof.
move=> gT p A G pG coGA defG; move/bigcupsP=> cChaA.
...
(* Aschbacher 23.7 *)
Lemma center_special_abelem : forall gT p (G : {group gT}),
  p.-group G -> special G -> p.-abelem 'Z(G).
Proof.
move=> gT p G pG [defPhi defG']; case: (pgroup_1Vpr pG) => [-> | [p_pr _ _]].
...
have sMT: M \subset T.
  have defT: 'F(G) = T.
    have p'F1: 'O_p^'('F(G)) = 1 by apply/trivgP; rewrite -p'G1 pcore_Fitting.
    case/dprodP: (nilpotent_pcoreC p (Fitting_nil G)) => _.
    by rewrite /= p_core_Fitting p'F1 mulg1.
...
case/splitsP: (SchurZass_split hallSN nsSN) => K.
...
(* B & G, Theorem 1.20 (Maeshke's Theorem) is covered by                      *)
(*   hall.Maeschke_abelem for internal action on elementary abelian subgroups *)
(*   mxrepresentation.mx_Maeschke for linear represenations on matrices.      *)
