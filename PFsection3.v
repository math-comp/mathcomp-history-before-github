...
(******************************************************************************)
(* This should be moved to vcharacter                                         *)
(* Defined here:                                                              *)
(*         dirr G ==  a predicate that checks if a class function is a signed *)
(*                    irreducible character:                                  *)
(*                 f \in dirr G <=> f \in irr G or f \in irr G.               *)
(*        dIirr G ==  a set of index for signed irreducible characters.       *)
(*         dchi i == the signed irreducible character of index i.             *)
(*        ndirr G == the sign function on signed index.                       *)
(*        dirr1 G == the unsigned index 0, i.e corresponds to the 1 character.*)
(* dirr_dIirr j f == the index n (or dirr1 if it does not exist) such that    *)
(*                    dchi n = f j.                                           *)
(*  dirr_constt f == the signed irreducible constituents of f                 *)
(*                    i \in f <=> [dchi i, f] > 0.                            *)
(*    to_dirr f i == the signed irreducible constituent of f that corresponds *)
(*                   to the constituent i.                                    *)
(*     of_irr f i == the irreducible constituent of f that corresponds to the *)
(*                   to signed constituent i.                                 *)
(******************************************************************************)
(* Defined here:                                                              *)
...
apply/eqP; rewrite eqN_eqC -subr_eq0; apply/eqP; rewrite -cfdot_sum_dchi.
...
rewrite subr_eq -(natrD _ 1%N) -eqN_eqC.
...
by rewrite !sub0r -!opprD -!natrD sposC_opp -(ltn_ltC _ 0) ltn0.
Qed.
...
Proof.
by move=> NZi NZj; rewrite inE dchi1 cfdot_bcTIirr_1 // /ltC eqxx.
Qed.
...
case: (boolP (_ j == _))=> [/eqP H1|_].
  ...
  by rewrite (negPf (ndirr_diff _)) (negPf JdJ') subr_eq0 -(eqN_eqC 0 1).
