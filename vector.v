...
Proof.
move=> I P v_ vs Hp; apply big_ind => //; first by apply mem0v.
by exact: memvD.
Qed.
...
by apply big_ind => // [|B C sAC]; rewrite ?subvf // subv_cap sAC.
Qed.
...
Lemma dim_span : forall m (t : m.-tuple V),  \dim (span t) <= m.
Proof.
elim=> [t|m Hrec]; first by rewrite [t]tuple0 span_nil dimv0.
...
Lemma span_mx_row : forall m (t : m.-tuple V) i, row i (span_mx t) = v2rv t`_i.
Proof.
move=> m t i; apply/matrixP=> i1 j1; rewrite !mxE.
by suff->: i1 = 0; [done | case: i1=> [[|i1] H] //; apply/eqP].
Qed.

Lemma span_vs2mx : forall m (t : m.-tuple V),  (vs2mx (span t) == span_mx t)%MS.
Proof.
move=> m t; rewrite /span !(big_nth 0) !big_mkord vs2mx_sum.
apply/andP; split.
...
Lemma span_mulmx : forall m (t : m.-tuple V) (s : 'I_m -> K),  
  \sum_(i < m) s i *: t`_i = 
     rv2v ((\row_(i < m) (s i)) *m (span_mx t)).
Proof.
move=> m t s; apply: v2rv_inj; rewrite rv2vK linear_sum mulmx_sum_row.
...
Lemma coord_is_linear : forall m (t : m.-tuple V), linear (coord t).
Proof.
move=> m t k u v; apply/ffunP=> i.
by unlock coord; rewrite !ffunE linearP mulmx_addl -scalemxAl !mxE.
Qed.
Canonical Structure coord_linear m (t : m.-tuple V) := Linear (coord_is_linear t).

Lemma coord_sumE : forall  m (t : m.-tuple V) I r (P : pred I) (F : I -> V) i,
 coord t (\sum_(j <- r | P j) F j) i = \sum_(j <- r | P j) coord t (F j) i.
Proof. 
move=> m t I r P F i; rewrite linear_sum.
elim: r=> [|j t1 Hrec]; first by rewrite !big_nil ffunE.
by rewrite !big_cons; case: (P j); rewrite ?ffunE Hrec.
Qed.

Lemma coord_span : 
  forall m (t : m.-tuple V) v, v \in span t -> v = \sum_(i < m) coord t v i *: t`_i.
Proof.
move=> m t v Hv; apply: v2rv_inj; rewrite linear_sum.
...
Lemma free_span_mx : forall m (t : m.-tuple V), free t = row_free (span_mx t).
Proof.
move=> m t; move: (span_vs2mx t).
by rewrite /free /dimv size_tuple; move/eqmxP->.
Qed.
...
Lemma freeP : forall m (t : m.-tuple V),  
  reflect
    (forall s, \sum_(i < m) s i *: t`_i =  0 -> s =1 fun _ => 0)
     (free t).
Proof.
move=> m t; rewrite free_span_mx -kermx_eq0.
...
Lemma free_coordt : forall m (t: m.-tuple V) (i j : 'I_m),  
...
Lemma freeE : forall m (t : m.-tuple V), free t = 
   forallb i : 'I_m, t`_i \notin span (drop i.+1 t).
Proof.
elim=> [[[]] //= _ | m IH t].
   by rewrite free_nil; apply/sym_equal; apply/forallP=> [] [[]].
...
Lemma freeNE : forall m (t : m.-tuple V), ~~(free t) = 
   existsb i : 'I_m, t`_i \in span (drop i.+1 t).
Proof.
move=> m t; rewrite freeE negb_forall.
...
Lemma is_span_span : forall vs m (t : m.-tuple V) v,  
...
Lemma size_is_basis : forall vs m (t : m.-tuple V), is_basis vs t -> \dim vs = m.
Proof.
rewrite /is_basis /is_span /free; move => vs m t; case/andP; do 2 move/eqP ->.
...
Definition vbasis vs := (fix loop (m : nat) vs {struct m} : m.-tuple V :=
  if m is m'.+1 return m.-tuple V then 
    [tuple of (vpick vs :: loop m' (vs :\: ((vpick vs)%:VS))%VS)] else tseq ([::]))
  (\dim vs) vs.
...
Lemma coord_basis : forall  v vs,  v \in vs -> 
  v = \sum_(i < \dim vs) coord (vbasis vs) v i *: (vbasis vs)`_i.
Proof.
...
exists (\sum_i (coord ([tuple of map f (vbasis vs)]) v i: K) *: (vbasis vs)`_i); split.
...
Canonical Structure ffunVectType := VectType K ffunVectMixin.
...
Lemma reg2rv_is_linear : linear reg2rv.
Proof. 
move=> k /= x y.
by rewrite /reg2rv raddfD scale_scalar_mx.
Qed.

Canonical Structure reg2rv_linear := Linear reg2rv_is_linear.
...
Canonical Structure regVectType := VectType R regVectMixin.
