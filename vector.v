...
(*      vecType K       == type for findim vector space structure.           *)
(*      vecMixin K      == builds the mixins containing the definition       *)
(*                         of a vector space over K.                         *)
(*      VecType T M     == packs the mixin M to build a vector space of type *)
(*                         vecType K. The field K will be infered from the   *)
(*                         mixin M. The underlying type T should have a      *)
(*                         lmodType canonical structure.                     *)
(*      ... inherite definition and proofs from lmodType.                    *)
(*      ... and proof many classical Lemmas on the vector space  structure   *)
(*      {vspace V}      == a vector space over V                             *)
(*     (V # W)%VS       == the simple product of two vecTypes                *)
...
(*       span l         == the linear span generated by the list l           *)
(*     vbasis vs        == a basis of vs                                     *)
(*     f @v: vs         == the image of vs by the linear function f          *)
(*    f @v^-1: vs       == the pre-image of vs by the linear function f      *)
(*     lker f           == the kernel of the linear function f               *)
(*     limg f           == the image of the linear function f                *)
(* (addv_pi1 V1 V2)     == projection of (V1 + V2)%VS onto V1                *)
(* (addv_pi2 V1 V2)     == projection of (V1 + V2)%VS onto V2                *)
(* (sumv_pi V_ P i)     == projection of (\sum_(i | P i) V_ i)%VS onto V_ i  *)
(*                                                                           *)
(*  Predicates:                                                              *)
...
Reserved Notation "'e_ p"  (at level 8, format "''e_' p").
Reserved Notation "''Hom' ( V , W )" (at level 0, format "''Hom' ( V , W )").
Reserved Notation "''End' ( V )" (at level 0, format "''End' ( V )").
Reserved Notation " f @v: V "  (at level 24).
Reserved Notation " f @v^-1: V "  (at level 24).
Reserved Notation "f \^-1" (at level 24).
Reserved Notation "A '#' B" (at level 40).
Reserved Notation "A '#^' n" (at level 50).
...
Lemma subsetv_refl : reflexive subsetv.
Proof. by move=> vs; exact: submx_refl. Qed.
Hint Resolve subsetv_refl.

Lemma subsetv_trans : transitive subsetv.
Proof. by move=> vs1 vs2 vs3; exact: submx_trans. Qed.

Lemma subsetv_anti : antisymmetric subsetv.
...
Lemma addv_sub : forall vs1 vs2 vs3,
...
by apply subsetv_anti; rewrite addv_sub Hs subsetv_refl addvSl.
Qed.
...
Proof.
by move=> vs;apply:subsetv_anti;apply/andP;split;[apply:subsetvf|apply:addvSl].
Qed.

Lemma addvf : right_zero fullv addv.
Proof.
by move=> vs;apply: subsetv_anti;apply/andP;split;
...
move=> v vs1 vs2; apply: (iffP idP); last first.
  by move=> [v1 [v2 [Hv1 Hv2 ->]]]; apply: addv_mem.
rewrite {1}/in_mem /= /subsetv /addv /= !genmxE addsmxE.
case/submxP; move=> x Hx.
exists (rv2v (lsubmx x *m vs2mx vs1)); exists (rv2v (rsubmx x *m vs2mx vs2)); split.
...
Qed.
Implicit Arguments addv_memP [v vs1 vs2].
...
apply: (iffP addv_memP) => [ [v1 [v2 [Hv1 Hv2 ->]]] | [v_ [H1v ->]]].
...
Lemma sumv_mem : forall P (vs_ : I -> V) (Vs_ : I -> {vspace V}),
...
by apply subsetv_anti; rewrite sub_capv Hs subsetv_refl capvSr.
Qed.
...
Proof.
move=> vs;apply: subsetv_anti;apply/andP;split; [apply: capvSl| apply: subset0v].
Qed.

Lemma capv0 : right_zero 0%:VS capv.
Proof.
move=> vs;apply: subsetv_anti; apply/andP; split; 
                                     [apply: capvSr| apply: subset0v].
Qed.
...
move=> P vs vs_; apply: (iffP idP) => [Hvs i Pi | Hvs].
  by apply: (subsetv_trans Hvs); rewrite (bigcapv_inf Pi).
by apply big_prop => // [|B C sAC]; rewrite ?subsetvf // sub_capv sAC.
Qed.
...
by rewrite /directv_def /= andbC -andbA subsetv0.
Qed.
...
apply/eqP; rewrite -subsetv0 -{2}TiQ_0 capvS //=; apply/subsetv_sumP=> j /=.
...
move => vs_; split.
 ...
 move: Hsum.
 rewrite !(bigD1 _ Pi) /=.
 by (rewrite dv ?Hu_ ?Hv_ ?sumv_mem //; first by case/andP);
...
Lemma span_subsetl : forall l vs, all (fun v => v \in vs) l = (span l <= vs)%VS.
Proof.
move => l vs; apply/allP/subsetvP=> H v.
  move/coord_span ->; apply: memv_sum => i _.
...
Definition vbasis vs := (fix loop (m : nat) vs {struct m} :=
  if m is m'.+1 then vpick vs :: loop m' (vs :\: ((vpick vs)%:VS))%VS else [::])
  (\dim vs) vs.
...
Hint Resolve subsetv_refl.
Implicit Arguments subsetvP [K V vs1 vs2].
Implicit Arguments sumv_sup [K V I P vs vs_].
Implicit Arguments subsetv_sumP [K V I P vs_ vs].
...
Definition lapp_rep_morph_proof : linear lapp_rep.
Proof. by move=> a x y; apply/rowP=> k; rewrite !(castmxE, mxE). Qed.
Canonical Structure lapp_rep_morph:= Linear lapp_rep_morph_proof.
...
Definition linearMixin := VectMixin lapp_rep_morph_proof lapp_rep_bij.
Canonical Structure linearVect (phV : phant V) (phW : phant W) := 
  VectType R linearMixin.

Variables (phV : phant V) (phW : phant W) (f : linearVect phV phW).

Lemma hom_linear_proof : linear f.
Proof.
by move=> a x y; rewrite /fun_of_lapp linearP mulmx_addl 
                        -scalemxAl linearP.
Qed.
Canonical Structure hom_linear := Linear hom_linear_proof.

Definition lapp_of_fun g := LinearApp (lin1_mx ((v2rv W) \o g \o (rv2v V))).
...
by move=> a u v; rewrite /h /= linearP Hg linearP.
Qed.
...
case: (pickP P) => [i Pi | P0]; last first.
  by rewrite !big_pred0 // => _; apply: subsetvf.
rewrite (cardD1x Pi) (bigD1 i) //=; move/Hrec=> {n1 Hrec} /= Hrec.
rewrite [(\bigcap_(i0 | P i0) _)%VS](bigD1 i) //=.
apply: (subsetv_trans (limg_cap _ _ _)).
...
exists (\sum_i (coord (map f (vbasis vs)) v i: K) *: (vbasis vs)`_i); split.
  apply: memv_sum=>  [] [i Hi] _; apply: memvZl; apply: memv_span.
...
apply/memv_imgP/memv_imgP=> [] [v [Hv ->]].
  by exists v; split => //; apply: (subsetv_trans Hv); exact: diffvSl.
move: Hv; rewrite -{1}(addv_diff_cap_eq vs (lker f)).
case/addv_memP=> v1 [v2 [Hv1 Hv2 ->]]; exists v1; split => //.
...
have F1: is_span (f @v: vs) (map f l).
...
move/subsetvP => Hvw; apply/subsetvP => z Hz.
...
by apply: subsetv_anti; rewrite -!(limg_ker0 _ _ Hf) H subsetv_refl.
Qed.

End LinearImage.

Notation " f @v: V " := (fun_of_limg f V).
Notation " f @v^-1: V " := (lpre_img f V).
...
apply: addvS; last by apply: subsetv_refl.
apply: limg_monotone.
...
move=> f v vs; apply/idP/idP; last first.
  ...
  by move=> Hi; apply: (subsetv_trans Hi); exact: capvSl.
...
rewrite lpre_imgE; apply/addv_memP; exists v1; exists (v - v1); split=> //.
  apply: memv_img; rewrite memv_cap Hv.
  by rewrite limgE; apply: memv_img; apply: subsetvf.
...
Inductive subv_of : predArgType := Subv x & x \in vs.
Definition sv_val u := let: Subv x _ := u in x.
Canonical Structure subv_subType :=
  Eval hnf in [subType for sv_val by subv_of_rect].
Definition subv_eqMixin := Eval hnf in [eqMixin of subv_of by <:].
Canonical Structure subv_eqType := Eval hnf in EqType subv_of subv_eqMixin.
Definition subv_choiceMixin := [choiceMixin of subv_of by <:].
Canonical Structure subv_choiceType :=
  Eval hnf in ChoiceType subv_of subv_choiceMixin.

Lemma subvP : forall u, sv_val u \in vs.
Proof. exact: valP. Qed.
Lemma subv_inj : injective sv_val.
Proof. exact: val_inj. Qed.
Lemma congr_subv : forall u v, u = v -> sv_val u = sv_val v.
Proof. exact: congr1. Qed.

Definition subv_zero := Subv (mem0v vs).
Definition subv_opp u := Subv (memvNr (subvP u)).
Definition subv_add u v := Subv (memvD (subvP u) (subvP v)).

Lemma subv_addA : associative subv_add.
Proof. by move=> u v w; apply: val_inj; exact: addrA. Qed.
Lemma subv_addC : commutative subv_add.
Proof. by move=> u v; apply: val_inj; exact: addrC. Qed.
Lemma subv_add0 : left_id subv_zero subv_add.
Proof. move=> u; apply: val_inj; exact: add0r. Qed.
Lemma subv_addN : left_inverse subv_zero subv_opp subv_add.
Proof. move=> u; apply: val_inj; exact: addNr. Qed.

Definition subv_zmodMixin := 
  GRing.Zmodule.Mixin subv_addA subv_addC subv_add0 subv_addN.
Canonical Structure subv_zmodType :=
  Eval hnf in ZmodType subv_of subv_zmodMixin.

Definition subv_scale k (u : subv_of) := Subv (memvZl k (valP u)).

Lemma subv_scaleA : forall k1 k2 u, 
  subv_scale k1 (subv_scale k2 u) = subv_scale (k1 * k2) u.
Proof. by move=> *; apply: val_inj; exact: scalerA. Qed.
Lemma subv_scale1 : left_id 1 subv_scale.
Proof. by move=> *; apply: val_inj; exact: scale1r. Qed.
Lemma subv_scale_addr : forall k, {morph (subv_scale k) : x y / x + y}.
Proof. by move=> k u v; apply: val_inj; exact: scaler_addr. Qed.
Lemma subv_scale_addl : forall u, {morph (subv_scale)^~ u : k1 k2 / k1 + k2}.
Proof. by move=> u k1 k2; apply: val_inj; exact: scaler_addl. Qed.

Definition subv_lmodMixin := 
  GRing.Lmodule.Mixin subv_scaleA subv_scale1 subv_scale_addr subv_scale_addl.
Canonical Structure subv_lmodType :=
  Eval hnf in LmodType K subv_of subv_lmodMixin.

Definition subv_v2rv (u : subv_of) :=
  \row_(j < size(vbasis vs)) (coord (vbasis vs) (sv_val u) j).

Lemma subv_v2rv_morph_p : linear subv_v2rv.
Proof.
...
Canonical Structure subv_v2rv_morph := Linear subv_v2rv_morph_p.

Lemma subv_v2rv_bij : bijective subv_v2rv.
Proof.
...
exists (fun r => Subv (v_in r)).
  move=> v1; apply: val_inj; rewrite /subv_v2rv /v.
...
Definition subv_VectMixin := VectMixin subv_v2rv_morph_p subv_v2rv_bij.
Canonical Structure subv_vectType := VectType K subv_VectMixin.

End SubVectorType.

Definition lappE := (zero_lappE, comp_lappE, scale_lappE, add_lappE, sum_lappE).
...
Notation "A # B" := (prodVector A B) : vspace_scope.
...
Fixpoint expVector (n : nat) {struct n} :=
  if n is n1.+1 then (V # (expVector n1))%VS else matrixVectType K 0 0.
...
Definition vsolve_eq ms :=
  ((ev_of_tuple veq) \in (evf feq) @v: ms)%VS.
