...
\paragraph*{}
L'énoncé du théorème de Cayley-Hamilton donne une relation entre les polynômes et les matrices, deux objets mathématiques différents. Pour pouvoir formaliser l'énoncé du théorème, il va falloir tout d'abord définir ces deux structures. Il est aussi question de polynôme caractéristique. La définition du polynôme caractéristique fait appel à celle de déterminant. En effet, le polynôme caractéristique est le déterminant d'une matrice de polynômes. La définition du déterminant nécessite celle des sommes et produits indexés, ainsi que celle des groupes de permutations.\newline
...
\end{verbatim}
Dans ce lemme, étant donnés deux ensembles \texttt{c} et \texttt{r}, une partition de \texttt{r} est donnée par les deux ensembles $\mathtt{r \cap c}$ et $\mathtt{r \cap \bar{c} }$. La somme des éléments indexés par \texttt{r} peut être donc décomposée en sommes des éléments indexés par ces deux ensembles. 
\paragraph*{}
...
\end{verbatim} 
Il est à remarquer que dans la structure ring ne contient pas la propriété de commutativité de la multiplication de l'anneau. Si l'anneau des matrices n'est pas commutatif, celui des polynômes peut être commutatif si l'anneau de ses coefficients l'est aussi. Dans la preuve, d'une part la règle de Cramer sur les matrices n'est vrai que si l'anneau des coefficients de la matrice est commutatif. Pour appliquer donc cette règle pour l'anneau des matrices de polynômes, il faut que celui-ci soit commutatif, ce qui nécessite que l'anneau des coefficients des polynômes soit commutatif. D'autre part, les polynômes de matrices sont construit sur l'anneau des matrices, qui n'est pas commutatif. L'anneau des polynômes peut se définir sur une structure d'anneau commutatif ou non commutatif. \newline
La structure d'anneau commutatif est définie au dessus de celle d'anneau et en lui ajoutant la propriété de commutativité de son opération de multiplication.
\begin{verbatim}
...
\paragraph*{}
Grâce à ces définitions, les structures d'anneau de polynômes et de matrices se construise sur celle d'anneau. Les constructeurs de ces structures prennent en paramètre un anneau et retournent l'anneau correspondant. Ainsi, pour définir les polynômes de matrices il suffira de passer comme paramètre au constructeur de la structure de polynômes l'anneau des matrices. La même méthode s'applique dans le sens inverse pour définir les matrices de polynômes. \newline
Un autre avantage est d'avoir la même notation pour les opérations d'anneau (addition, multiplication et opposé) sur les polynômes et les matrices. Nous aurons ainsi la même notation pour ces opérations, dans le cas des polynômes, des matrices ou de l'anneau de base. Ce qui est proche des notations utilisées en mathématiques classiques où il n'y a pas de différence entre l'addition de polynômes ou celle de matrices. Ceci se fera à l'aide du mécanisme de \texttt{Canonical Structure} ~\cite{coqman} de \coq. Un exemple d'utilisation de ce mécanisme dans l'exemple des matrices est donné la prochaine section.
...
\paragraph*{}
Après avoir définis le type des matrices et pour construire la structure d'anneau sur le type des matrices carrées, il va falloir prouver les axiomes d'anneaux sur ce type. 
\begin{verbatim}
...
\end{verbatim} 
Dans l'énoncé du lemme les matrices \texttt{A} et \texttt{B} ne sont pas déclaré de type \texttt{matrix\_ring} mais simplement de type \texttt{matrix}, \coq{} a inféré automatiquement la structure d'anneau et a accepté l'utilisation de la notation pour l'addition d'anneau dans la partie gauche de l'égalité.
\paragraph*{}
...
\end{verbatim} 
La fonction \verb|coef| est de type \verb|nat -> R|. Étant donné un entier $i$, \verb|coef| retourne l'élément d'indice \verb|i| dans la séquence des coefficients du polynômes p , si i est inférieure à la taille de cette séquence. Elle retourne 0 dans le cas contraire. \newline
...
\end{equation}
Suivant le schéma (\ref{horner-sch}) l'évaluation ne dépend que de la séquence des coefficients et se définit par récurrence sur cette séquence. La fonction d'évaluation peut être défie par récurrence comme suit :
\begin{verbatim}
