...
Après développement de l'application de $\phi$ dans l'égalité ci-dessus, nous obtenons la formule (\ref{proof_start}).\newline
...
Des librairies de base sont définies dans \ssr. C'est une hiérarchie de structure pour travailler avec les types décidables et en particulier les types finis. La structure \verb|eqType| définit les types munis d'une égalité décidable qui s'injecte dans celle de Leibniz.
\begin{verbatim}
...
\paragraph*{}
La bibliothèque sur les déterminants utilise la formule de Leibniz pour définir le déterminant. Ce choix est motivé par le fait que dans cette formule il est question de groupe de permutations et qu'une formalisation des permutations (groupe, signature ...) a été déjà développée dans le cadre du travail sur les groupes finies []. Etant donné une une matrice carrée $A$ de dimension $n$, le déterminant est donnée par :
\begin{equation}
