...
(* Axiomatic definition of field type : commutative ring with inverse *)
(* for all no null elements *)
...
Notation Local "x ^-1" := (inv x) : ring_scope.
...
Proof.
by move=> x *; move/(congr1 (mul x)); rewrite mulVf // mulr0; apply: nonzero1r.
Qed.
Lemma neq0_mul : forall x y : F, x <> 0 -> y <> 0 -> (x * y <> 0).
Proof.
by move=> x *; move/(congr1 (mul (x^-1))); rewrite mulr0 mulrA mulfV // mul1r.
Qed.
Lemma invfK : forall x : F, x <> 0 -> (x^-1)^-1 = x.
Proof.
by move=> x Hx; apply: (mulfI (neq0I Hx)); rewrite mulVf // mulfV //; apply: neq0I.
Qed.
Lemma invf1 : 1^-1 = 1 :> F.
...
Notation "x ^-1" := (Field.inv x) : ring_scope.
...
(* Definition of the field structure with list choice function *)
...
End FieldPlus.
Notation field_p := FieldPlus.fieldPlus.

Definition choose := FieldPlus.choose_.

Section FieldPlusEquations.
Variable F : field_p.

Lemma chooseP : forall (P : seq F -> bool) (E : exists x, P x), P (choose E).
Proof. by rewrite /choose; case: F. Qed.
Lemma chooseEq :
 forall (P Q : seq F -> bool) (Ep : exists x, P x) (Eq : exists x, Q x),
 P =1 Q -> choose Ep = choose Eq.
Proof. by rewrite /choose; case: F. Qed.

End FieldPlusEquations.
