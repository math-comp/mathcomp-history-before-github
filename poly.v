...
Definition pcoef_head h i (p : poly_of (Phant R)) := let: tt := h in p`_i.
...
Arguments Scope pcoef_head [_ _ nat_scope ring_scope _].
Notation "{ 'poly' T }" := (poly_of (Phant T)).
Notation pcoef i := (pcoef_head tt i).
...
Lemma polyCK : cancel polyC (pcoef 0).
Proof. by move=> c; rewrite [pcoef 0 _]coefC. Qed.
...
Lemma coef_add_poly p q i : (add_poly p q)`_i = p`_i + q`_i.
Proof.
rewrite coef_poly; case: leqP => //; rewrite leq_maxl => /andP[le_p_i le_q_i].
by rewrite !nth_default ?add0r.
Qed.

Lemma coef_opp_poly p i : (opp_poly p)`_i = - p`_i.
Proof.
...
Definition mixin := ZmodMixin add_polyA add_polyC add_poly0 add_poly_opp.

End PolynomialZmodule.
End PolynomialZmodule.

Canonical poly_zmodType (R: ringType) :=
  Eval hnf in ZmodType {poly R} (PolynomialZmodule.mixin R).
Canonical polynomial_zmodType (R: ringType) :=
  Eval hnf in [zmodType of polynomial R for (poly_zmodType R)].

Section PolynomialZmodTheory.

Variable R : ringType.
Implicit Types (a b c x y : R) (p q : {poly R}).
...
Lemma coef_add p q i : (p + q)`_i = p`_i + q`_i.
Proof. exact: PolynomialZmodule.coef_add_poly. Qed.

Lemma coef_opp p i : (- p)`_i = - p`_i.
Proof. exact: PolynomialZmodule.coef_opp_poly. Qed.

Lemma coef_sub p q i : (p - q)`_i = p`_i - q`_i.
Proof. by rewrite coef_add coef_opp. Qed.

Canonical pcoef_additive i :=
  Additive ((fun p => (coef_sub p)^~ i) : additive (pcoef i)).

Lemma coef_natmul p n i : (p *+ n)`_i = p`_i *+ n.
Proof. exact: (raddfMn (pcoef_additive i)). Qed.

Lemma coef_negmul p n i : (p *- n)`_i = p`_i *- n.
Proof. by rewrite coef_opp coef_natmul. Qed.

Lemma coef_sum I r (P : pred I) (F : I -> {poly R}) k :
  (\sum_(i <- r | P i) F i)`_k = \sum_(i <- r | P i) (F i)`_k.
Proof. exact: (raddf_sum (pcoef_additive k)). Qed.

Lemma polyC_add : {morph polyC : a b / a + b}.
Proof. by move=> a b; apply/polyP=> [[|i]]; rewrite coef_add !coefC ?addr0. Qed.

Lemma polyC_opp : {morph polyC : c / - c}.
Proof. by move=> c; apply/polyP=> [[|i]]; rewrite coef_opp !coefC ?oppr0. Qed.
...
Proof. by rewrite /lead_coef size_opp coef_opp. Qed.
...
Lemma size_sum I r (P : pred I) (F : I -> {poly R}) :
...
Proof.
move=> ltqp; rewrite /lead_coef coef_add size_addl //.
...
Proof.
move=> p q r; apply/polyP=> i; rewrite coef_add !coef_mul_poly -big_split.
...
Canonical poly_ringType (R : ringType) :=
  Eval hnf in RingType {poly R} (PolynomialRing.mixin R).
Canonical polynomial_ringType (R : ringType) :=
...
Lemma coef_mul p q i : (p * q)`_i = \sum_(j < i.+1) p`_j * q`_(i - j)%N.
Proof. exact: PolynomialRing.coef_mul_poly. Qed.

Lemma coef_mul_rev p q i : (p * q)`_i = \sum_(j < i.+1) p`_(i - j)%N * q`_j.
...
have [-> | nz_p] := poly0Vpos p; first by rewrite lead_coef0 !mul0r coef0.
have [-> | nz_q] := poly0Vpos q; first by rewrite lead_coef0 !mulr0 coef0.
have ->: (size p + size q).-2 = (dp + dq)%N.
  by rewrite -(prednK nz_p) -(prednK nz_q) addnS.
have lt_p_pq: dp < (dp + dq).+1 by rewrite ltnS leq_addr.
rewrite coef_mul (bigD1 (Ordinal lt_p_pq)) ?big1 ?simp ?addKn //= => i.
rewrite -val_eqE neq_ltn /= => /orP[lt_i_p | gt_i_p]; last first.
  by rewrite nth_default ?mul0r //; rewrite prednK in gt_i_p.
rewrite [q`__]nth_default ?mulr0 //= -subSS -{1}addnS prednK //.
...
case: (poly0Vpos p) => [-> | nz_p]; first by rewrite mulr0 size_poly0.
rewrite (leq_trans (size_mul _ _)) // -predn_sub -!subn1 leq_sub2r // -addnS.
by rewrite -addn_subA ?leq_add2l // ltnW // -subn_gt0 (leq_trans nz_p).
Qed.

Lemma coef_Cmul c p i : (c%:P * p)`_i = c * p`_i.
Proof.
rewrite coef_mul big_ord_recl subn0.
by rewrite big1 => [|j _]; rewrite coefC !simp.
Qed.

Lemma coef_mulC c p i : (p * c%:P)`_i = p`_i * c.
Proof.
rewrite coef_mul_rev big_ord_recl subn0.
by rewrite big1 => [|j _]; rewrite coefC !simp.
Qed.

Lemma polyC_mul : {morph polyC : a b / a * b}.
Proof. by move=> a b; apply/polyP=> [[|i]]; rewrite coef_Cmul !coefC ?simp. Qed.
...
Lemma pcoef0_multiplicative : multiplicative (pcoef 0 : {poly R} -> R).
Proof.
split=> [p q|]; last by rewrite polyCK.
by rewrite [pcoef 0 _]coef_mul big_ord_recl big_ord0 addr0.
Qed.

Canonical pcoef0_rmorphism := AddRMorphism pcoef0_multiplicative.
...
Lemma scale_polyE a p : scale_poly a p = a%:P * p.
Proof.
apply/polyP=> n; rewrite coef_poly coef_Cmul.
by case: leqP => // le_p_n; rewrite nth_default ?mulr0.
Qed.

Lemma scale_polyA a b p : scale_poly a (scale_poly b p) = scale_poly (a * b) p.
...
Lemma scale_poly_addr a : {morph scale_poly a : p q / p + q}.
Proof. by move=> p q; rewrite !scale_polyE mulr_addr. Qed.

Lemma scale_poly_addl p : {morph scale_poly^~ p : a b / a + b}.
Proof. by move=> a b /=; rewrite !scale_polyE raddfD mulr_addl. Qed.

Lemma scale_poly_mull a p q : scale_poly a (p * q) = scale_poly a p * q.
...
Proof. by rewrite -PolynomialLalg.scale_polyE. Qed.

Lemma scale_poly1 a : a *: 1 = a%:P :> {poly R}.
Proof. by rewrite -mul_polyC mulr1. Qed.

Lemma coef_scaler a p i : (a *: p)`_i = a * p`_i.
Proof. by rewrite coef_poly; case: leqP => // le_p_n; rewrite nth_default ?mulr0. Qed.

Canonical pcoef_linear i :=
  AddLinear ((fun a => (coef_scaler a) ^~ i) : scalable (pcoef i : {poly R} -> R^o)).
...
Variable R : ringType.
Implicit Types (a b c : R) (p q : {poly R}).
...
Proof. by rewrite !polyseq_cons nil_poly eqxx /= polyseq1. Qed.
...
Proof.
apply/polyP=> i; rewrite coef_mul_rev coef_mul.
by apply: eq_bigr => j _; rewrite coefX commr_nat.
Qed.

Lemma coef_mulX p i : (p * 'X)`_i = (if (i == 0)%N then 0 else p`_i.-1).
Proof.
rewrite coef_mul_rev big_ord_recl coefX ?simp.
case: i => [|i]; rewrite ?big_ord0 //= big_ord_recl polyseqX subn1 /=.
by rewrite big1 ?simp // => j _; rewrite nth_nil !simp.
Qed.

Lemma coef_Xmul p i : ('X * p)`_i = (if (i == 0)%N then 0 else p`_i.-1).
Proof. by rewrite -commr_polyX coef_mulX. Qed.

Lemma poly_cons_def p a : poly_cons a p = p * 'X + a%:P.
Proof.
apply/polyP=> i; rewrite coef_cons coef_add coef_mulX coefC.
...
Lemma seq_mul_polyX p : p != 0 -> p * 'X = 0 :: p :> seq R.
Proof.
...
Lemma coef_Xn n i : 'X^n`_i = (i == n)%:R.
Proof.
by elim: n i => [|n IHn] [|i]; rewrite ?coef1 // exprS coef_Xmul ?IHn.
Qed.

Lemma seq_polyXn n : 'X^n = rcons (nseq n 0) 1 :> seq R.
Proof.
elim: n => [|n IHn]; rewrite ?polyseq1 // exprSr.
by rewrite seq_mul_polyX -?size_poly_eq0 IHn ?size_rcons.
Qed.

Lemma size_polyXn n : size 'X^n = n.+1.
Proof. by rewrite seq_polyXn size_rcons size_nseq. Qed.

Lemma commr_polyXn p n : GRing.comm p 'X^n.
Proof. by apply: commr_exp; exact: commr_polyX. Qed.

Lemma lead_coefXn n : lead_coef 'X^n = 1.
Proof. by rewrite /lead_coef nth_last seq_polyXn last_rcons. Qed.

Lemma polyseq_mulXn n p : p != 0 -> p * 'X^n = ncons n 0 p :> seq R.
Proof.
case: n => [|n] nz_p; first by rewrite mulr1.
elim: n => [|n IHn]; first exact: seq_mul_polyX.
by rewrite exprSr mulrA seq_mul_polyX -?nil_poly IHn.
Qed.

Lemma coef_mulXn n p i : (p * 'X^n)`_i = if i < n then 0 else p`_(i - n).
Proof.
have [-> | /polyseq_mulXn->] := eqVneq p 0; last exact: nth_ncons.
by rewrite mul0r !coef0 if_same.
Qed.

Lemma coef_Xn_mul n p i : ('X^n * p)`_i = if i < n then 0 else p`_(i - n).
Proof. by rewrite -commr_polyXn coef_mulXn. Qed.
...
Variable R : ringType.
Implicit Types (p q : {poly R}) (x a c : R) (s rs : seq R).
...
Lemma horner_sum I r (P : pred I) F x :
...
Definition com_coef p x := forall i, p`_i * x = x * p`_i.

Definition com_poly p x := x * p.[x] = p.[x] * x.

Lemma com_coef_poly p x : com_coef p x -> com_poly p x.
Proof.
move=> cpx; rewrite /com_poly !horner_coef big_distrl big_distrr /=.
by apply: eq_bigr => i _; rewrite /= mulrA -cpx -!mulrA commr_exp.
Qed.

Lemma com_poly0 x : com_poly 0 x.
Proof. by rewrite /com_poly !horner0 !simp. Qed.

Lemma com_poly1 x : com_poly 1 x.
Proof. by rewrite /com_poly !hornerC !simp. Qed.

Lemma com_polyX x : com_poly 'X x.
Proof. by rewrite /com_poly !hornerX. Qed.

Lemma horner_mul_com p q x : com_poly q x -> (p * q).[x] = p.[x] * q.[x].
Proof.
move=> com_qx.
...
by rewrite {}IHp -mulrA -com_qx mulrA -mulr_addl horner_amulX.
Qed.

Lemma horner_exp_com p x n : com_poly p x -> (p ^+ n).[x] = p.[x] ^+ n.
Proof.
move=> com_px; elim: n => [|n IHn]; first by rewrite hornerC.
by rewrite !exprSr -IHn horner_mul_com.
Qed.

Lemma hornerXn x n : ('X^n).[x] = x ^+ n.
Proof. by rewrite horner_exp_com /com_poly hornerX. Qed.
...
Proof. by rewrite !horner_lin_com addrN. Qed.
...
apply/polyP=> i; rewrite mulr_subr coef_sub coef_mulX coef_mulC !coef_poly.
...
Proof. by apply/polyP=> i; rewrite !(coef_map, coef_scaler) /= rmorphM. Qed.
...
Lemma map_com_poly p x : com_poly p x -> com_poly p^f (f x).
Proof. by rewrite /com_poly horner_map -!rmorphM // => ->. Qed.

Lemma map_com_coef p x : com_coef p x -> com_coef p^f (f x).
...
Lemma axiom p q : p * q = q * p.
Proof.
apply/polyP=> i; rewrite coef_mul coef_mul_rev.
...
Canonical poly_comRingType (R: comRingType) :=
  Eval hnf in ComRingType {poly R} (@PolynomialComRing.axiom R).
Canonical polynomial_comRingType (R: comRingType) :=
  Eval hnf in [comRingType of polynomial R for (poly_comRingType R)].
Canonical poly_algType (R: comRingType) :=
  Eval hnf in CommAlgType R {poly R}.
Canonical polynomial_algType (R: comRingType) :=
...
Proof. by rewrite horner_mul_com //; exact: mulrC. Qed.

Lemma horner_exp p x n : (p ^+ n).[x] = p.[x] ^+ n.
Proof. by rewrite horner_exp_com //; exact: mulrC. Qed.
...
Lemma poly_intro_unit p q : q * p = 1 -> poly_unit p.
Proof.
move=> pq1; apply/andP; split; last first.
  apply/unitrP; exists q`_0.
  by rewrite 2!mulrC -!/(pcoef 0 _) -rmorphM pq1 rmorph1.
...
Lemma map_poly_com p x : com_poly p^f (f x).
Proof. exact: map_com_poly (mulrC x _). Qed.
...
rewrite -cats1 big_cat big_seq1 /= mulrA rootE horner_mul_com; last first.
  by rewrite /com_poly horner_factor mulr_subl mulr_subr czt.
...
Lemma map_poly_comp (bR cR : ringType) (f : bR -> cR) (g : R -> bR) p :
  f 0 = 0 -> map_poly (f \o g) p = map_poly f (map_poly g p).
Proof.
move=> f0; apply/polyP => i; rewrite /map_poly !coef_poly.
...
Lemma poly_comp_multiplicative p : multiplicative (poly_comp p).
Proof.
...
apply/polyP=> i; rewrite coef_deriv coef_natmul !coef_Xn eqSS.
...
by rewrite !(coef_deriv, coef_add, coef_scaler) mulrn_addl mulrnAr.
Qed.
...
Proof.
apply/polyP=> i; rewrite !(coef_deriv, coef_add, coef_mul).
...
Proof.
apply/polyP=>i; rewrite coef_derivn coef_natmul !coef_Xn.
...
Proof.
apply/polyP=> i.
by rewrite coef_natmul coef_nderivn coef_derivn -mulrnA bin_ffact.
Qed.
...
Proof.
apply/polyP=> i; rewrite coef_nderivn coef_natmul !coef_Xn.
...
Proof.
move=> k p q; apply/polyP=> i.
by rewrite !(coef_nderivn, coef_add, coef_scaler) mulrn_addl mulrnAr.
Qed.
...
Proof.
apply/polyP=> i; rewrite coef_nderivn !coef_add !coef_mulX coefC.
...
by rewrite nderivn_amulX !horner_lin_com /= mulr_addl -!mulrA -exprSr cxh.
Qed.
