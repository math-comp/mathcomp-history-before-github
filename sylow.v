...
Require Import fintype paths connect groups div action.
Require Import cyclic zp normal finfun finset.
...
Let zp := zp_group (prime_pos_nat prime_p).
...
by rewrite -{1}[x]make_zp_val IHm /= modn_add2m.
Qed.
...
rewrite zprot_to_morph setE; case/andP; move/ffun_onP=> Hfx prodf1.
apply: IHi; rewrite setE {i} rot1_prod_over_zp prodf1 andbT.
...
rewrite -(card_dimset injF); apply: eq_card => t; rewrite setE.
apply/andP/imsetP=> [[]|[t1 Yt1 ->{t}]].
  ...
  apply/pffun_onP; split=> u.
    move/eqP=> dFtu; rewrite /= setE; apply/eqP=> Du; case: dFtu.
    by rewrite {u}Du ffunE /f eqxx Yt invg1.
  move/imageP=> [v /= vnot1 ->{u}]; rewrite ffunE /f /=.
  by rewrite setE in vnot1; rewrite (negbET vnot1).
...
move/eqP: sup1 => Dt11; have{Xt1} Xt1: forall u, t1 u \in H.
  ...
  by apply: Xt1; apply/imageP; exists u; rewrite //= setE sup_u.
...
pose t1 : {ffun zp -> elt} := [ffun => 1].
have Zt1: t1 \in Z.
  apply/andP; split; [|rewrite /= setE; apply/andP; split].
...
rewrite setE; case/andP; move/ffun_onP=> /= Ht prodt _.
pose x := t 1; exists x; rewrite Ht /=.
have Dt: t _ = x by move=> u; rewrite /x -{2}(fixt u (setAP _)) ffunE mulg1.
have: orderg x %| p.
...
by rewrite Dt -(gexpn1 x) ffunE Dx1 (eqP (orderg_expn1 _)).
Qed.
...
Definition sylow (A : {set elt}) : pred {set elt} := 
...
have F1: forall L x, x \in K -> sylow K L -> sylow K (L :^ x).
  ...
  apply/andP; split.
    apply/subsetP=> y; rewrite /sconjg setE; gsimpl => Lyx.
...
have F1: forall H L x, sylow H L -> sylow (H :^ x) (L :^ x).
  ...
  rewrite /sylow 2!card_sconjg -(eqP card_L) eqxx andbT.
  apply/subsetP=> y; rewrite /sconjg setE; gsimpl => Lyx.
  by move: (subsetP sLH _ Lyx) => Hxy; rewrite setE.
move=> H L x; apply/idP/idP => Hx1; first exact: F1.
rewrite -(@sconj1g _ L) -(mulgV x) sconjgM.
by rewrite -(@sconj1g _ H) -(mulgV x) sconjgM F1.
Qed.


Lemma sylow1_rec: forall i (L : {group elt}), i < n ->
  L \subset K -> (#|L| == p ^ i)%N ->
  exists H : {group elt}, 
    [&& L \subset H, H \subset K, L <| H & #|H| == p ^ i.+1]%N.
Proof.
...
have: p %| #|K / L|.
  ...
  rewrite /indexg (@mpl _ _ (trans_action elt) _ _ _ _ prime_p (eqP cardL)).
    by congr (_ %% _); apply: eq_card=> x; rewrite !setE andbC.
...
have sLH : L \subset H.
  ...
  apply/setIP; split; first by rewrite setE coset_of_id // group1.
  rewrite dom_coset //; exact: (subsetP (norm_refl L)).
rewrite sLH subsetIr /=; apply/andP; split.
  by apply/subsetP=> x; case/setIP; case/mpreimP=> *; rewrite -dom_coset.
rewrite -(@LaGrange _ L) // -card_quotient /= -/H; last first.
  by apply/subsetP=> x; do 2!case/setIP; rewrite dom_coset.
rewrite (eqP cardL) mulnC -{}Czbar_p.
apply/eqP; congr (_ * _)%N; apply: eq_card => xbar.
apply/imsetP/idP=> [[x Hx ->{xbar}]|].
  by rewrite 3!setE -andbA in Hx; case/andP: Hx.
case/cyclicP=> m; move/eqP=> <-{xbar}; rewrite {sLH}/H.
case/quotientP: Kzbar => z [Kz Nz ->{zbar}].
by exists (z ** m); rewrite 3?setE morphE // dom_coset ?groupE ?cyclicnn.
Qed.

Lemma sylow1: forall i j (L : group elt), i <= j -> j <= n ->
  L \subset K -> #|L| == (p ^ i)%N ->
  exists H: group elt, [&& L \subset H, H \subset K & #|H| == p ^ j]%N.
Proof.
...
move=> H1; case/and4P => Hhh1 Hh1k Hnh1 Hch1; exists H1.
by rewrite Hh1k Hch1 (subset_trans Hlh).
Qed.
...
Theorem sylow1_subset : forall i (L : {group elt}),
  L \subset K -> #|L| == (p ^ i)%N -> 
  exists P : {group elt}, (L \subset P) && sylow K P.
Proof.
...
Lemma sylow2 : forall (H L : {group elt}) i, H \subset K ->
...
have F2:  indexg L K %% p = #|lS0| %% p. 
  ...
  rewrite (@mpl _ _ (trans_action elt) _ _ _ _ prime_p Hch).
    by congr (_ %% _); apply: eq_card => Ha; rewrite inE /= !setE /= andbC.
  apply: intro_closed=> A B; first exact: orbit_csym.
...
case: F3 => Hx; case/setIP=> Hx2; rewrite setE => Hx3.
case/imsetP: Hx2 => x Kx /= -> in Hx3.
by exists x; rewrite Kx /=; apply: (@act_fix_sub _ L).
Qed.
...
Theorem sylow2_subset: forall i (L P : {group elt}),
...
by move/normalP: Hnp; move /(_ _ Kx)<-; rewrite -EP1.
Qed.
...
Definition gsylow A : pred {group elt} := [eta sylow A].

Theorem sylow3_div : #|gsylow K| %| #|K|.
Proof.
case sylow1_cor => H Hh.
suff ->: #|gsylow K| = #|orbit (gconj_action elt) K H|.
...
case/andP: (Hh) => F2 F3.
rewrite -(eq_card (setE (gsylow p K))).
...
set S0 := predI _ _.
have F5: H \in S0.
  apply/andP; split; last by rewrite /= setE.
...
have F6: forall x, x \in S0 -> x = H.
  ...
  have F7: H \subset nLK. 
    apply/subsetP => y Hy.
    rewrite setE; apply/andP; split; last exact: (subsetP F2).    
    by rewrite setE /= -{2}(Hl _ Hy) subset_refl.
  have F8 : sylow p nLK H.
    apply: (@sylow_subset _ _ K)  => //.
    apply/subsetP => y.
    by rewrite setE; case/andP.
  have F9 : sylow p nLK L.
    apply: (sylow_subset (K := K)) => //;
      last by apply/subsetP => y; rewrite setE; case/andP.
    apply/subsetP => /= x Hx.
    apply/setIP; split.
