...
Lemma gdcop_rec_map:
  forall (F : fieldType) (R : idomainType) (f : {rmorphism F -> R})
         (p q : {poly F}) n,
    (map_poly f (gdcop_rec p q n)) =
    (gdcop_rec (map_poly f p) (map_poly f q) n).
Proof.
move => F R f p q n.
elim: n p q => [|n IH] => /= p q.
...
Lemma gdcop_map:
  forall (F : fieldType) (R : idomainType) (f : {rmorphism F -> R})
         (p q : {poly F}),
    (map_poly f (gdcop p q)) = (gdcop (map_poly f p) (map_poly f q)).
Proof.
move => F R f p q.
...
Canonical Structure subfext_unitRing := 
  Eval hnf in UnitRingType subFExtend 
    (FieldUnitMixin subfx_fieldAxiom subfx_inv0).

Canonical Structure subfext_comUnitRing := 
  Eval hnf in [comUnitRingType of subFExtend].

Canonical Structure subfext_idomainType :=
  Eval hnf in IdomainType subFExtend
              (FieldIdomainMixin (@FieldMixin _ _ subfx_fieldAxiom subfx_inv0)).

Canonical Structure subfext_fieldType :=
...
rewrite rmorphD !{1}rmorphM map_polyC /= /poly_comp -horner_map rmorphM.
...
have: (gcdp p2 (q ^ iota) %= ('X - y%:P)).
 apply/andP; split; last first.
  ...
  by rewrite !(horner_poly_comp, horner_lin) subrr mulr0 add0r.
 rewrite -[_ - _]mul1r.
...
move => suff K x Hx.
apply: anti_leq.
by rewrite suff //= -{1}[K]prodv1 -(mulfV Hx) prodv_inj prodvA
           suff // invr_neq0.
Qed.

Definition polyOver (K : {vspace L}) := [pred p : {poly L} | all (mem K) p].

Lemma polyOverP : forall (K : {vspace L}) (p : {poly L}),
...
Lemma polyOver_subset : forall K E p, (K <= E)%VS ->
...
case: ex_minnP.
case.
 by rewrite /P ltn0 muln1 big_ord1 expr0 prodv1 !ltnn.
...
Lemma poly_for_eq : forall v, v \in Fadjoin -> v = (poly_for_Fadjoin v).[x].
Proof.
move => v Hv.
...
case => // m _ Hm _ xK.
apply/eqP.
rewrite eqSS -leqn0 -ltnS Hm // !big_ord_recl big_ord0 expr1 expr0 addv0 
        prodv1.
apply/orP; right.
...
move/prednK: (elementDegreegt0 K x) sizep => <- sizep sump.
...
apply/andP; split; apply: (subv_trans _ HFxE).
 rewrite /Fadjoin.
 move/prednK: (elementDegreegt0 K x) <-.
 by rewrite big_ord_recl expr0 prodv1 addvSl.
move: HxED.
rewrite /Fadjoin.
move/prednK: (elementDegreegt0 K x) <-.
case: (elementDegree K x).-1 => [|d].
 by rewrite expr1.
move => _.
...
apply/idP/idP.
 apply: contraL.
 move/eqP ->.
 by rewrite coprimep0 -size_poly_eq1 size_minPoly eqSS -lt0n elementDegreegt0.
move => Hderiv.
...
have hmD : forall t, polyOver K t ->
 ...
 rewrite coef0 {1}(coef_map [linear of (DerivationExtend _ _ _)]) 
         ?{1}linear0 //= {1}DerivationExtended.
  apply/eqP.
  by rewrite -memv_ker KD0.
 apply: (subv_trans _ (subsetKFadjoin _ _)).
 by apply: Ht.
by rewrite (DerivationSeparable DED) // !hmD ?{1}compose_polyOver ?{1}minPolyOver //
...
rewrite /poly_comp horner_coef size_map_poly.
...
have -> : t *: ('X - y%:P) + x%:P =
          ('X + x%:P) \Po (t *: ('X - y%:P)).
  by rewrite poly_comp_addl poly_comXp poly_comCp.
...
rewrite -(eqp_dvdl _ (mulp_gcdl _ _ _)) -{2}['X]mul1r dvdp_mul2r
        -?size_poly_eq0 ?size_polyX // dvdp1 gcdp_eqp1.
apply: contraR troot.
