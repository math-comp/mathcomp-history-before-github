...
Require Import normal sylow cyclic.
...
Lemma transpD: forall x y z: d, x != z -> y != z -> transp x y z = z.
by move => x y z D1 D2; case transpP => // HH;
   [case/negP: D1 | case/negP: D2]; rewrite HH.
Qed.

Lemma transpCom: forall x y z t: d, uniq (Seq x y z t) -> 
 commute (transp x y) (transp z t).
move=> x y z t Uniq; apply/conjg_fpP.
...
Lemma perm_ind: forall P: _ -> Prop, 
 P 1 -> (forall x y, P (transp x y)) -> 
 (forall p1 p2: permType d, P p1 -> P p2 -> P (p1 * p2)) ->
 forall p, P p.
Proof.
move=> P H1 Htrans Hmorph.
have HP0: forall p : permType  d, card {x : d, p x != x} = 0 -> P p.
  move => p Hp; suff ->: p = 1 by exact: H1.  
  apply: eq_fun_of_perm => z; rewrite perm1.
  suff: {x : d, p x != x} z = false; first by rewrite s2f; move/eqP.
  by rewrite (card0_eq Hp).
move=> p; elim: (card _) {-2}p (leqnn (card {x, p x != x})) => {p}[| n Hrec] p Hp.
  by apply: HP0; move: Hp; case: card.
case Ex: (set0b {x : d, p x != x}); first by apply: HP0; move: Ex; rewrite /set0b; move/eqP.
case/set0Pn: Ex => x; rewrite s2f => Hx.
pose p1 := p * transp x (p x).
have Hp1: card {x : d, p1 x != x} <= n.
  have F1: forall y, p y = y -> p1 y = y.
    move=> y Hy; rewrite /p1 permM Hy.
    case transpP => //; first by move => <-.
    by move=> Hpx; apply: (@perm_inj _ p); rewrite -Hpx.  
  have F2: p1 x = x by rewrite /p1 permM transpR.
  have F3: subset {x : d, p1 x != x} (setD1 {x : d, p x != x} x).
    apply/subsetP => z; rewrite s2f /p1 permM.
    case transpP => HH1 HH2; rewrite /setD1 s2f.
    - case E1: (_ == _); first by case/negP: Hx; rewrite {1}(eqP E1) HH1.
      by rewrite HH1 E1.
  ...
  by move: Hp; rewrite (cardD1 x) s2f Hx.
suff ->: p = p1 * transp x (p x) by apply Hmorph => //; apply Hrec.
by rewrite /p1 -mulgA transp2 mulg1.
Qed.
